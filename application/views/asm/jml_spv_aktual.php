				...
				<table id="mytable" class="table table-hover table-bordered">
					...
					<?php
						...
						foreach ($query->result() as $val) {
							...
							//
							//$sqlInc = $this->asm_model->getIncCurrent($val->SPV_Code, $tanggal);
							...
							{
								$tanggal_1 = $val->Join_Date;
							}
							$tanggal_2 = date('Y-m-d');
							$waktu_1 = strtotime("$tanggal_1");
					...
					?>
						...
						<?php
							...
							$sqlBreakdown = $this->asm_model->getAllDsrBySpv($val->SPV_Code);
							$nod = 0;
					...
					?>
						...
						<?php
							...
							foreach ($sqlDummy->result() as $rDum) {
								...
								$sqlDumLs = $this->asm_model->getAppsLsAllDsr($rDum->DSR_Code, $tanggal_old);
								...
								//
								//$qrD = $this->asm_model->getIncCurrentDsr($rDum->DSR_Code, $tanggal);
								//if($qrD->num_rows() > 0)
								//{
									//$qrDt = $qrD->row();
								...
								$sqlDums = $this->asm_model->getAppsLsAllDsr($rDum->DSR_Code, $tanggal);
								...
								//datediff
								$tgl1 = $rDum->Efektif_Date;
								if($tgl1 == "0000-00-00")
								{
									$tgl1 = $rDum->Join_Date;
								}
								$tgl2 = date('Y-m-d');
								$timeStart_1 = strtotime("$tgl1");
								$timeEnd_1 = strtotime("$tgl2");
								// Menambah bulan ini + semua bulan pada tahun sebelumnya
								$diff_1 = $timeEnd_1-$timeStart_1;
								// menghitung selisih bulan
								$numBulan_1 = $diff_1 / 86400 / 30;
						?>
						...
						<tr class="tr_detail2" style='font-size: 11px; display: none;'>
							...
							<td class="text-right"> <?php echo round($numBulan_1); ?></td>
							<td class="text-right"> -</td>
							...
							<td>
								<?php
									if($rtg == "Under Perform 1" OR $rtg == "Under Perform 2"  OR $rtg == "Under Perform" OR $rtg == "Warning" OR $rtg == "Zero Account")
									{
									...
									}elseif($rtg == "Excellent 1" OR $rtg == "Excellent 2" OR $rtg == "Excellent 3" OR $rtg == "Excellent 4")
									{
							...
							<td>
								<?php
									if($rtg_dums == "Under Perform 1" OR $rtg_dums == "Under Perform 2"  OR $rtg_dums == "Under Perform" OR $rtg_dums == "Warning" OR $rtg_dums == "Zero Account")
									{
									...
									}elseif($rtg_dums == "Excellent 1" OR $rtg_dums == "Excellent 2" OR $rtg_dums == "Excellent 3" OR $rtg_dums == "Excellent 4")
									{
