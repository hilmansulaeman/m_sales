...
<div class="box box-primary">
	<div class="box-header with-border">
		...
		<h3 class="box-title">Data DSR</h3>			  
	</div>
			...
			<div style="height:500px;overflow-y:scroll;">
				<table class="table table-hover table-bordered" id="example">
					<thead>
						<tr>
							<th colspan="3"></th>
							<th colspan="2" class="text-center"><?php echo date('M-Y'); ?></th>
							<th colspan="2" class="text-center"><?php echo date('M-Y', strtotime('-1 month')); ?></th>
							<th colspan="2"></th>
						</tr>
						<tr>
							<th>NO</th>
					...
					<?php
						...
						foreach ($query->result() as $val) {
							$sqlAps = $this->rsm_model->getApsByDsr($val->DSR_Code, $val->Product, $tanggal);
							if($sqlAps!="")
							{
								$rowAps = $sqlAps->row();
								$inc = $rowAps->inc;
								$rts = $rowAps->rts;
								if($inc > 0){$inc = $inc;}else{$inc = 0;}
								if($rts > 0){$rts = $rts;}else{$rts = 0;}
							}
							
							$sqlApr = $this->rsm_model->getAprByDsr($val->DSR_Code, $date);
							if($sqlApr->num_rows > 0)
							{
								$rowApr = $sqlApr->row();
								$app = $rowApr->approve;
								$rej = $rowApr->reject;
								$rating = $rowApr->Rating;
							}
							else
							{
								$app = 0;
					...
					?>
						<tr>
							<td><?php echo ++$no; ?></td>
							<td><?php echo $val->DSR_Code; ?></td>
							<td><?php echo $val->Name; ?>, <?php echo $val->Product; ?></td>
							<td><?php echo $inc; ?></td>
							<td><?php echo $rts; ?></td>
							<td><?php echo $app; ?></td>
							<td><?php echo $rej; ?></td>
					...
					?>
						<tr>
							...
							<td><?php echo $jml_app; ?></td>
							<td><?php echo $jml_rej; ?></td>
							<td></td>
							<td></td>
						</tr>
					</tbody>
