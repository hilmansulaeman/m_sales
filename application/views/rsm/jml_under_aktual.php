				...
				<table class="table table-hover table-bordered">
					...
					<?php
						...
						foreach ($query->result() as $rows) {
							$sql_ls = $this->rsm_model->getApssByAsm($rows->DSR_Code, $tanggal_old);
							...
							//realtime
							//$sqlReal = $this->rsm_model->getApsRealtime($rows->DSR_Code, $tanggal);
							//$rReal = $sqlReal->row();
							//$totInc = $rReal->inc_cc + $rReal->inc_edc + $rReal->inc_sc;
							//$totRts = $rReal->rts_cc + $rReal->rts_edc + $rReal->rts_sc;

							$sql = $this->rsm_model->getApssByAsm($rows->DSR_Code, $tanggal);
							if($sql->num_rows() > 0)
							{
								$rows2 = $sql->row();
								$inc = $rows2->Inc_cc + $rows2->Inc_edc + $rows2->Inc_sc;
								$rts = $rows2->Rts_cc + $rows2->Rts_edc + $rows2->Rts_sc;
					...
					?>
						<tr>
							<td><?php echo ++$no; ?></td>
							<td><b>ASM</b> <a href="#" onclick="Detail('<?php echo $rows->DSR_Code ?>');"><?php echo $rows->Name; ?></a></td>
							<td>-</td>
					...
					<?php
						$nous = 0;
						$sql_us = $this->rsm_model->getSpvByAsm($rows->DSR_Code);
					...
					?>
						<tr class="tr_<?php echo $rows->DSR_Code; ?>" style="font-size: 11px; display: none">
					...
					?>
						<?php
						$sqlDummySpv = $this->rsm_model->getDummyApsByAsm($rows->DSR_Code,$tanggal_old);
						...
						//reals 
						/*$sqlDummySpvCur = $this->rsm_model->getDummyCurrently($rows->DSR_Code,$tanggal);
						...
						$sqlDummySpv2 = $this->rsm_model->getDummyApsByAsm($rows->DSR_Code,$tanggal);
					...
					?>
						<tr class="tr2_<?php echo $rows->DSR_Code; ?>" style="font-size: 11px; display: none;">
							<td class="text-right"><?php echo ++$nous; ?></td>
