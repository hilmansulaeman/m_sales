...
<div class="box box-primary">
	...
	<div class="box-header with-border">
		...
		<h3 class="box-title">Decision CC <?php echo $position; ?></h3>

	</div>
	<div class="panel-body">
		<div class="row">
			<div class="col-sm-12">
				<a href="cc/export" target="_blank"><button type="button" id="btn-export" class="btn btn-primary" style="padding:5px;"><i class="fa fa-file-excel-o"></i> Export Data</button></a>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $("#data-table-customer").DataTable({
			...
			responsive: true,
			ajax: {
				url: "<?php echo site_url('decision/cc/get_data') ?>",
				type: 'POST',
				/*data: function ( data ) {
	...
	function filter_data() {
		...
		$.ajax({
			url: "<?php echo site_url('decision/cc/filter_data') ?>",
			type: "POST",
			data: formData,
			contentType: false,
			processData: false,
	...
	function view_spv(sales, pos, name) {
		...
		$.ajax({
			url: "<?php echo site_url('decision/cc/detailSPV'); ?>/" + sales + "/" + pos,
			type: "POST",
	...
	function view_detail(sales, status, name) {
		...
		$.ajax({
			url: "<?php echo site_url('decision/cc/detailActual'); ?>/" + sales + "/" + status,
			type: "POST",
