				...
				<ul class="nav nav-stacked">
					...
					<li>
						<a href="javascript:void(0);"
							onclick="detail_cc('APPROVED')">APPROVED<span
								class="pull-right badge bg-green"><?php echo $br_cc->approve; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);"
							onclick="detail_cc('ADD_ON')">ADD ON<span
								class="pull-right badge bg-green"><?php echo $br_cc->add_on; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);" onclick="detail_cc('INPROCESS')">INPROCESS<span
								class="pull-right badge bg-green"><?php echo $br_cc->inprocess; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);" onclick="detail_cc('CANCEL')">CANCEL<span
								class="pull-right badge bg-green"><?php echo $br_cc->cancel; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);"
							onclick="detail_cc('DECLINED')">DECLINE<span
								class="pull-right badge bg-green"><?php echo $br_cc->decline; ?></span></a>
					</li>
				</ul>
			</div>
		</div>
...
<script>
	function detail_cc(status) {
		var sales_code = $('#sales_code').val();
		$('#modalDetail').modal('show');
		$.ajax({
			url: '<?php echo site_url(); ?>decision/cc/detailActual/' + sales_code + '/' + status,
			type: 'POST',
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							<input type="text" name="date" id="dt_startDate" class="form-control date-input"
								value="<?php echo $this->input->post('date'); ?>" placeholder="Y-m"
								autocomplete="off" />
						</div>
					</div>
