...
<div class="box box-primary">
	...
	<div class="box-header with-border">
		...
		<h3 class="box-title">Decision SC <?php echo $position; ?></h3>
	</div>
...
$(document).ready(function() {
    table = $("#data-table-customer").DataTable({
		...
		responsive:true,
		ajax: {
		url: "<?php echo site_url('decision/sc/get_data') ?>",
		type:'POST',
		/*data: function ( data ) {
...
function filter_data()
{
    ...
    $.ajax({
        url : "<?php echo site_url('decision/sc/filter_data')?>",
        type: "POST",
		data: formData,
		contentType: false,
        processData: false,
...
function view_spv(sales, pos, name)
{
	...
	$.ajax({
		url:"<?php echo site_url('decision/sc/detailSPV'); ?>/" + sales + "/" + pos,
		type:"POST",
...
function view_detail(sales, status, name)
{
	...
	$.ajax({
		url:"<?php echo site_url('decision/sc/detailActual'); ?>/" + sales + "/" + status,
		type:"POST",
