...
<div class="box box-primary">
	...
	<div class="box-header with-border">
		...
		<h3 class="box-title">Decision Corporate <?php echo $position; ?></h3>
	</div>
	<div class="panel-body">
		<div class="row">
			<div class="col-sm-12">
			<a href="corp/export"><button type="button" id="btn-export" class="btn btn-primary" style="padding:5px;"><i class="fa fa-file-excel-o"></i> Export Data</button></a>
...
$(document).ready(function() {
    table = $("#data-table-customer").DataTable({
		...
		responsive:true,
		ajax: {
		url: "<?php echo site_url('decision/corp/get_data') ?>",
		type:'POST',
		/*data: function ( data ) {
...
function filter_data()
{
    ...
    $.ajax({
        url : "<?php echo site_url('decision/corp/filter_data')?>",
        type: "POST",
		data: formData,
		contentType: false,
        processData: false,
...
function view_spv(sales, pos, name)
{
	...
	$.ajax({
		url:"<?php echo site_url('decision/corp/detailSPV'); ?>/" + sales + "/" + pos,
		type:"POST",
...
function view_detail(sales, status, name)
{
	// console.log(status);
	$('#modalActual').modal('show');
	$.ajax({
		url:"<?php echo site_url('decision/corp/detailActual'); ?>/" + sales + "/" + status,
		type:"POST",
