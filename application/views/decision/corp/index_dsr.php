<?php
	...
	{
		$tgl = $this->session->set_userdata('groupDateFilter', $this->session->userdata('groupDate'));
		$periode = $periode;
	}
	else
	{
		$tgl = $this->session->set_userdata('groupDateFilter', $this->input->post('date'));
		$periode = date('M / Y', strtotime($this->input->post('date')));
	}
	
	//$rowCount = $counter->row();
	$br_corp = $breakdown_corp;
?>
				...
				<ul class="nav nav-stacked">
					...
					<li>
						<a href="javascript:void(0);"
							onclick="detail_corp('APPROVED')">APPROVE<span
								class="pull-right badge bg-green"><?php echo $br_corp->approve_corp; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);" onclick="detail_corp('CANCEL')">CANCEL<span
								class="pull-right badge bg-green"><?php echo $br_corp->cancel_corp; ?></span></a>
					</li>
					<li>
						<a href="javascript:void(0);"
							onclick="detail_corp('DECLINED')">DECLINE<span
								class="pull-right badge bg-green"><?php echo $br_corp->decline_corp; ?></span></a>
					</li>
				</ul>
			</div>
		</div>
...
<script>
	function detail_corp(status) {
		var sales_code = $('#sales_code').val();
		$('#modalDetail').modal('show');
		$.ajax({
			url: '<?php echo site_url(); ?>decision/corp/detailActual/' + sales_code + '/' + status,
			type: 'POST',
