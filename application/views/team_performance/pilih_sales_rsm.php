<table class="table table-hover" id="example2" style="font-size:10px !important;">
	...
	{
		$asm_code = $row->DSR_Code;
	?>
		<tr>
			<td><b>ASM</b> <?php echo $row->Name; ?></td>
			<td>
			...
			<?php
			$sqlSpv = $this->db->query("SELECT DSR_Code as spv_code, Name as namaSpv FROM `internal`.`data_sales_structure` WHERE ASM_Code='$asm_code' AND Position='SPV' AND Status='ACTIVE' ORDER BY namaSpv ASC");
			foreach($sqlSpv->result() as $rowSpv)
			{
			...
			<tr>
				...
				?>
				<tr>
					<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<b>DSR</b> <?php echo $rowDsr->namaDs; ?></td>
					<td>
