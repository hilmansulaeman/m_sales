<?php
	...
	$dataBS = $dataGetDataBS; //IS = Input => Belum System
...
<script>
	function detailMerchant(status, part)
	{
		...
		$.ajax({
            url:'<?php echo site_url(); ?>incoming/merchant/det_breakdown_merchant_leader/'+ sales_code + '/' + sales + '/' + status + '/' + part + '/popup',
            type:'POST',
		...
		// window.open('<?php echo site_url('incoming/merchant/det_breakdown_merchant_leader'); ?>/' + sales_code + '/' + sales + '/' + status + '/' + part, '_blank');
	}
</script>
