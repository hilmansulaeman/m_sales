					...
					<table id="data-table-customer" class="table table-hover">
						<thead>
							<tr>
								<th width="5%">No</th>
								<th>Data</th>
								<?php if ($this->session->userdata('position') == 'ASM') { ?>
									<th>SPV Name</th>
								<?php } ?>
								<?php if ($this->session->userdata('position') == 'RSM') { ?>
									<th>SPV Name</th>
									<th>ASM Name</th>
								<?php } ?>
								<?php if ($this->session->userdata('position') == 'BSH') { ?>
									<th>SPV Name</th>
									<th>ASM Name</th>
									<th>RSM Name</th>
								<?php } ?>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $("#data-table-customer").DataTable({
			...
			responsive: true,
			ajax: {
				url: "<?php echo site_url('recruitment_approval/get_data') ?>",
				type: 'POST',
				/*data: function ( data ) {
