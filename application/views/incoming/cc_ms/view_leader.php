<?php
	...
	$inprocess = $query->send == null ? 0 : $query->inprocess;
	$duplicate = $query->send == null ? 0 : $query->duplicate;
	$rts = $query->send == null ? 0 : $query->rts;
	$cancel = $query->send == null ? 0 : $query->cancel;
	$reject = $query->send == null ? 0 : $query->reject;
?>
	...
	<div class="box-header with-border">
		...
		<div class="box-body">
			...
			<ul class="nav nav-stacked">
				<li><a href="javascript:void(0);" onclick="view_detail('SEND');">SEND DIKA <span class="pull-right badge bg-blue"><?php echo $send ?></span></a></li>
...
<script>
	function view_detail(status)
	{
		...
		$.ajax({
            url:"<?php echo site_url('incoming/cc_ms/detail_leader_popup'); ?>/" + sales_code + "/" + nik + "/" + status,
            type:'POST',
