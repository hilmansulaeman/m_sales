...
<div class="box box-primary">
	...
	<div class="box-header with-border">
		...
		<?php if (in_array($position, $array_structure)) { ?>
			<a href="<?php echo site_url('incoming_new/cc_ms/view_leader'); ?>" class="btn btn-primary">View Detail</a>
		<?php } ?>
	</div>
	<div class="panel-body">
		...
		<div class="row">
			<div class="col-sm-2">
				...
				<a href="<?= site_url('incoming_new/cc_ms/export') ?>"><button type="button" id="btn-export" class="btn btn-primary" style="padding:5px;"><i class="fa fa-file-excel-o"></i> Export Data</button></a>
			</div>
			<div class="col-sm-10">
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		table = $("#data-table-customer").DataTable({
			...
			responsive: true,
			ajax: {
				url: "<?php echo site_url('incoming_new/cc_ms/get_data') ?>",
				type: 'POST',
				/*data: function ( data ) {
	...
	function filter_data() {
		...
		$.ajax({
			url: "<?php echo site_url('incoming_new/cc_ms/filter_data') ?>",
			type: "POST",
			data: formData,
			contentType: false,
			processData: false,
	...
	function view_spv(sales, pos, name) {
		...
		$.ajax({
			url: "<?php echo site_url('incoming_new/cc_ms/detailSPV'); ?>/" + sales + "/" + pos,
			type: "POST",
	...
	function view_detail(sales, nik, status, name) {
		...
		console.log(status);
		$.ajax({
			url: "<?php echo site_url('incoming_new/cc_ms/detail_leader_popup'); ?>/" + sales + "/" + nik + "/" + status,
			type: "POST",
	...
	function view_detail_ms(sales, nik, name) {
		...
		// console.log(status);
		$.ajax({
			url: "<?php echo site_url('incoming_new/cc_ms/detail_leaderMS_popup'); ?>/" + sales + "/" + nik,
			type: "POST",
			data: $("#frmspv1").serialize(),
			success: function(data) {
				$("#pop1").html('');
