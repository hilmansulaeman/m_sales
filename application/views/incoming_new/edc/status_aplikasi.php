...
<a href="<?= site_url('incoming/edc/get_status/'.$dari.'/'.$sampai) ?>" class="btn btn-primary">Refresh <i class="fa fa-refresh"></i></a>
...
<div class="box box-primary">
	<div class="box-header with-border">
		<div class="col-md-6">
			<h4>Setoran Aplikasi EDC | <?=$this->session->userdata('position')?></h4>
		</div>
		<div class="col-md-6">
	...
	<div class="box-body">
		...
		<!-- Add the bg color to the header using any of the bg-* classes -->
			...
			<ul class="nav nav-stacked">
				<?php
					foreach($status_aplikasi->result() as $key) {
						$status_edc = $key->status;
						$status1_edc = strtolower($status_edc);
						$status2_edc = str_replace('_', ' ', $status1_edc);
						if ($status_edc == 'SUBMIT_TO_DIKA') {
							$count_edc = $br_edc->submit_to_dika;
						} elseif ($status_edc == 'RETURN_TO_SALES') {
							$count_edc = $br_edc->returned;
						} elseif ($status_edc == 'PENDING_FU') {
							$count_edc = $br_edc->pending_fu;
						} elseif ($status_edc == 'SUBMIT_TO_BCA') {
							$count_edc = $br_edc->submit_to_bca;
						} elseif ($status_edc == 'RETURN_FROM_BCA') {
							$count_edc = $br_edc->return_from_bca;
						} elseif ($status_edc == 'RESUBMIT_TO_BCA') {
							$count_edc = $br_edc->resubmit_to_bca;
						} elseif ($status_edc == 'RESUBMIT_TO_DIKA') {
							$count_edc = $br_edc->resubmit_to_dika;
						} elseif ($status_edc == 'CANCEL') {
							$count_edc = $br_edc->cancel;
						} elseif ($status_edc == 'REJECT') {
							$count_edc = $br_edc->reject;
						}
				...
				?>
					<li>
						<a href="<?php echo site_url('incoming/edc/index/'.$status1_edc.'/'.$dari.'/'.$sampai); ?>">
							<?=strtoupper($status2_edc)?> <span class="pull-right badge bg-blue"><?=$count?></span>
						</a>
					</li>
				<?php } ?>
			</ul>
...
<Script>
	...
	function submit_filter()
	{
		...
		if(tgl1 == "" || tgl2 ==  ""){
			...
			window.location='<?php echo base_url() ?>filter_incoming_edc/index/'+ tgl1 +'/'+ tgl2;
		}
