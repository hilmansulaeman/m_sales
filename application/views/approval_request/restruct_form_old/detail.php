            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table id="data-request" class="table table-responsive table-bordered table-striped">
                        ...
                        <form action="<?= site_url();?>approval_request/restruct_form/approve/<?=$request_id?>" method="POST" onsubmit="return validate(this);">
                        <tbody>
                            <?php 
                                $no = 1;
                                foreach ($request_user->result() as $r) {
                                    ...
                                    if($position == 'ASM') {
                                        $sales_name = $r->Checker_Name;
                                    }
                                    elseif($position == 'RSM') {
                                        $get_status = $this->db->query("SELECT * FROM data_request_approval WHERE Request_User_ID = '$request_user_id' AND Position = 'ASM' AND Status = '1'")->row();
                                        $sales_name = $get_status->Sales_Name;
                                    }
                                    elseif($position == 'BSH') {
                                        $get_status = $this->db->query("SELECT * FROM data_request_approval WHERE Request_User_ID = '$request_user_id' AND Position = 'RSM' AND Status = '1'")->row();
                                        $sales_name = $get_status->Sales_Name;
                                    }
                            ?>
                            <tr>
                                <?php
                                    $q_cek = $this->db->query("SELECT * FROM internal_sms.data_request_approval WHERE Sales_Code = '$username' AND Status = '0'")->num_rows();
                                ...
                                <td>
                                    <?php
                                        $cek = $this->db->query("SELECT * FROM data_request_approval WHERE Request_ID = '$request_id' ORDER BY Request_Approval_ID ASC LIMIT 1 ")->row();
                                    ...
                                    ?>
                                    <label class="label label-xs label-info"><?=$label?> <?=$sales_name?></label>
                                </td>
                            </tr>
                            <!-- end foreach-->
                            <?php } ?>
                    ...
                    ?>
                        ...
                        <input type="submit" class="btn btn-danger" formaction="<?php echo site_url();?>approval_request/restruct_form/reject/<?=$request_id?>" name="reject" value="Tolak" onclick="return confirm('Yakin tolak data ini?')">
                    <?php } else { ?>
                        <a href="<?= site_url()?>approval_request/restruct_form" class="btn btn-primary">Back</a>
                    <?php } ?>
