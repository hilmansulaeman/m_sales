...
{	
	...
	// LEADER PAGE
		...
		function get_data()
		{
			...
			if($position == 'ASM'){
				$get_dummy_spv_IS = $this->merchant_model->getDataInputDummy($nik,$date_from,$date_to,'asm','IS', $source);
				$get_dummy_spv_BS = $this->merchant_model->getDataInputDummy($nik,$date_from,$date_to,'asm','BS', $source);
				
				//Total DSR
				$total_dsr2  = $this->merchant_model->getDataInputDummy($nik, $date_from, $date_to, 'asm', 'IS', $source);

				$get_dummy_spv_PR = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PR', $source);
				$get_dummy_spv_PI = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PI', $source);
				$get_dummy_spv_PRTS = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PRTS', $source);
				$get_dummy_spv_PS = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PS', $source);
				$get_dummy_spv_PC = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PC', $source);
				$get_dummy_spv_PPS = $this->merchant_model->getAppProcessingDummy($nik,$date_from,$date_to,'asm','PPS', $source);

				$getTotalsReceived  = $this->merchant_model->getTotalsProcessingDummy($nik, $date_from, $date_to, 'asm', 'received', $source);
				$getTotalsInprocess = $this->merchant_model->getTotalsProcessingDummy($nik, $date_from, $date_to, 'asm', 'inprocess', $source);
				$getTotalsRTS  		= $this->merchant_model->getTotalsProcessingDummy($nik, $date_from, $date_to, 'asm', 'rts', $source);
				$getTotalsSend  	= $this->merchant_model->getTotalsProcessingDummy($nik, $date_from, $date_to, 'asm', 'send', $source);
