...
class Cc_ms extends MY_Controller
{
	...
	function index()
	{
		...
		//load view
		$this->template->set('title', 'Summary Incoming CCMS');
	...
	function get_data()
	{
		...
		foreach ($query->result() as $row) {
			...
			$getIncoming = $this->cc_model_new->getDataInputLeader($var, $sales, $date_from, 'ms')->row(); // Reguler
			// $getTotalCcms = $this->cc_model_new->getDataCountCcms($var, $sales); // Reguler
		...
		if ($position == 'ASM') {
			...
			$getIncomingDummy = $this->cc_model_new->getDataInputDummy($nik, $date_from, 'spv', 'ms')->row(); // Mobile Sales
	...
	function get_data_spv()
	{
		...
		foreach ($query->result() as $row) {
			...
			$getIncoming = $this->cc_model_new->getDataInputLeader($var, $sales, $date_from, 'ms')->row(); // Mobile Sales
			if (in_array($row->Position, $array_dsr)) {
				$total_dsr = 0;
			} else {
				$total_dsr 	 = $getIncoming->total_ms == NULL ? 0 : $getIncoming->total_ms;
	...
	function view_leader()
	{
		...
		$data['query'] = $this->cc_model_new->detBreakdownInputLeader($data['var'], $username, $date_from)->result();
	...
	function detail_leader_popup($sales_code, $sales)
	{
		...
		$data['query'] 	= $this->cc_model_new->detBreakdownInputLeader($sales_code, $sales, $tgl1)->result();
	...
	function detail_leaderMS_popup($sales_code, $sales)
	{
		...
		$data['query'] 	= $this->cc_model_new->detBreakdownInputMSLeader($sales_code, $sales, 'reg', $tgl1);
	...
	function detail_leader($sales_code, $sales, $status)
	{
		...
		$data['status'] 	= $status;
		$data['query'] 	= $this->cc_model_new->detBreakdownInputLeader($sales_code, $sales, $status, 'reg', $tgl1);
		//load view
	...
	function export()
	{
		$position 	= 't3.' . $this->session->userdata('position') . '_Code';
		$nik 		= $this->session->userdata('sl_code');
		...
		$query = $this->cc_model_new->get_dataExportCcms($position, $nik, $from)->result();
		// $this->unit->run($query, 'is_array', 'Unit Testing Munculkan Field Type Pada Saat Export');
		// echo $this->unit->report();
		// die;
...
<?php
			echo "<meta http-equiv='refresh' content='0; url=" . site_url() . "incoming_new/cc_ms'>";
			...
			foreach ($query as $data) { // Lakukan looping pada variabel sn
				...
				$sheet->setCellValue('O' . $numrow, $data->Type);
			...
			header("Content-Disposition: attachment; filename=Detail Incoming CCMS.xlsx");
	...
	public function unitTestingShowData()
	{
		$position 	= $this->session->userdata('position');
		$nik 		= $this->session->userdata('sl_code');
		$date_from 	= $this->session->userdata('date_from');
		...
		// echo $where;
		// die;
		$query = $this->cc_model_new->get_dataTable($where)->result();
		// var_dump($query);

		$this->unit->run($query, 'is_array', 'Unit Testing Show DATA CCMS BY Filter Baru');
		echo $this->unit->report();

		// foreach ($query as $row) {
		// 	echo $sales = $row->DSR_Code;
		// }
	}
	...
	// 		$getIncoming = $this->cc_model_new->getDataInputLeader($var, $sales, $date_from, $date_to, 'ms'); // Reguler
	// 		// $getTotalCcms = $this->cc_model_new->getDataCountCcms($var, $sales); // Reguler
	// 		if (in_array($row->Position, $array_dsr)) {
	// 			$total_dsr = 0;
	// 		} else {
	...
	// 		$getIncomingDummy = $this->cc_model_new->getDataInputDummy($nik, $date_from, $date_to, 'asm', 'ms'); // Mobile Sales
	// 		$total_dsr_dummy = $getIncomingDummy->total_ms == null ? 0 : $getIncomingDummy->total_ms;
	// 		$input_dummy = $getIncomingDummy->total == null ? 0 : $getIncomingDummy->total;
	// 		// $input_dummy 	  = $getIncomingDummy->total == null ? 0 : $getIncomingDummy->total;
	// 		// $send_dummy       = $getIncomingDummy->send == null ? 0 : $getIncomingDummy->send;
