...
class Pemol extends MY_Controller
{
	function __construct()
	{
		...
		$this->load->model('incoming_new/pemol_model');
	}
	...
	function index()
	{
		...
		$this->template->load('template', 'incoming_new/pemol/index', $data);
	}
	...
	function get_data()
	{
		...
		$array_leader = array('BSH', 'RSM', 'ASM');
		...
		$query = $this->pemol_model->get_dataTable($where);
		// $this->unit->run($query, 'is_array', 'Unit Testing Menampilkan Field BCA Mobile & MyBCA');
		// echo $this->unit->report();
		// die;
		// $this->unit->run($query, "is_array", "Unit Testing Show Data PEMOL Filter Baru");
		// echo $this->unit->report();
		// die;
		$data = array();
		...
		foreach ($query->result() as $row) {
			...
			$getPemol = $this->pemol_model->get_pemol($var, $sales, $date_from)->row();
			// $getPemol = $this->pemol_model->get_pemol($var, $sales);
			...
			if (in_array($row->Position, $array_dsr)) {
				...
				$dsr_active 	 = $getPemol->dsr_input == NULL ? 0 : $getPemol->dsr_input;
			}
			if (in_array($position, $array_leader)) {
				$cols = array(
					...
					'<span title="BCA Mobile" class="badge bg-yellow">' . number_format($getPemol->bcaM) . '</span>',
					'<span title="My BCA" class="badge bg-blue">' . number_format($getPemol->mBca) . '</span>',
					'<span title="Total" class="badge bg-green">' . number_format($actual) . '</span>',
		...
		if ($position == 'ASM') {
			// $get_pemol_dummy_spv = $this->pemol_model->get_pemol_dummy($var, $nik, $date_from, 'spv');
			$get_pemol_dummy_spv = $this->pemol_model->get_pemol_dummy($nik, $date_from, 'spv')->row();
			$actual_dummy_spv = $get_pemol_dummy_spv->total;
			$dsr_active_dummy = $get_pemol_dummy_spv->dsr_input;
			...
			$data[] = array(
				...
				'<span title="BCA Mobile" class="badge bg-yellow">' . number_format($get_pemol_dummy_spv->bcaM) . '</span>',
				'<span title="My BCA" class="badge bg-blue">' . number_format($get_pemol_dummy_spv->mBca) . '</span>',
		...
		$output = array(
			...
			"recordsTotal" => $this->pemol_model->count_dataTable($where),
			"recordsFiltered" => $this->pemol_model->count_dataTable($where),
			"data" => $data,
		);
	...
	function detail($sales, $pos)
	{
		...
		//load view
		$this->load->view('incoming_new/pemol/detail');
	}
	...
	function get_data_spv()
	{
		...
		// $dsr_position = "('DSR','SPG','SPB','FO','Funding Officer','RO','Relationship Officer','Mobile Sales')";
		$dsr_position = "('DSR','SPG','SPB','FO','Funding Officer','RO','Relationship Officer')";
		$asm_position = "('SPV', 'DSR','SPG','SPB','FO','Funding Officer','RO','Relationship Officer')";
		...
		foreach ($query->result() as $row) {
			...
			// $getPemol = $this->pemol_model->get_pemol($var, $sales, $date_from);
			// $actual = $getPemol->total;
			// $dsr_active = $getPemol->dsr_active;
			// $dsr_input = $getPemol->dsr_input;
			...
			$getPemol = $this->pemol_model->get_pemol($var, $sales, $date_from)->row();
			$actual = $getPemol->total;
			$dsr_active = $getPemol->dsr_input;
			$dsr_input = $getPemol->dsr_input;

			if (in_array($row->Position, $dsr_position_arr)) {
				$dsr_active = 0;
			}
		...
		if ($pos == 'ASM') {
			...
			$data[] = array(
				...
				'<span title="DSR Active" class="badge bg-black">' . number_format($dsr_active_dummy) . '</span>',
				// '<span title="DSR Input" class="badge bg-red">' . number_format($dsr_input_dummy) . '</span>',
				'<span title="Total Input" class="badge bg-red">' . number_format($dsr_input_dummy) . '</span>',
				'<span title="BCA Mobile" class="badge bg-yellow">' . number_format($get_pemol_dummy_spv->bcaM) . '</span>',
				'<span title="My BCA" class="badge bg-blue">' . number_format($get_pemol_dummy_spv->mBca) . '</span>',
				'<span title="Actual" class="badge bg-green">' . number_format($actual_dummy_spv) . '</span>',
				'<a href="javascript:void(0);" onclick="view_spv(\'' . $nik . '\', `SPV`, `DUMMY SPV`)" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>'
	...
	function export()
	{
		...
		$sheet->setCellValue('O1', "TGL INPUT");
		...
		$query = $this->pemol_model->get_pemol_export($position, $nik, $from)->result();
		// $this->unit->run($query, 'is_array', 'Unit Testing Export Data Incoming Pemol');
		// echo $this->unit->report();
		// die;
		// echo "<pre>";
		// var_dump($query);
...
<?php
			echo "<meta http-equiv='refresh' content='0; url=" . site_url() . "incoming_new/pemol'>";
			...
			foreach ($query as $data) { // Lakukan looping pada variabel sn
				$maskingname = $this->maskingname($data->NO_Rekening);
				...
				$sheet->setCellValue('M' . $numrow, $data->Status_Sales);
				$sheet->setCellValue('N' . $numrow, $data->Source);
				$sheet->setCellValue('O' . $numrow, $data->Tgl_Input);
			...
			header("Content-Disposition: attachment; filename=Detail Incoming PEMOL.xlsx");
