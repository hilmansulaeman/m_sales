...
class Tmcc extends MY_Controller
{
	...
	function get_data()
	{
		...
		if ($position == 'ASM') {
			...
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmcc($nik, $date_from, 'spv', 'ms')->row(); // Mobile Sales
	...
	function get_data_spv()
	{
		...
		if ($pos == 'ASM') {
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmcc($nikSes, $date_from, 'spv', 'ms')->row(); // Mobile Sales
	...
	function view_leader()
	{
		...
		$data['query'] = $this->cc_model_new->get_dataInputLeaderTmcc($data['var'], $username, $date_from, 'ms')->row(); // Reguler
...
<?php
			...
			foreach ($query as $data) { // Lakukan looping pada variabel sn
				...
				$sheet->setCellValue('B' . $numrow, $data->Ms_Name);
