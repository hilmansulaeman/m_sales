...
class Pl extends MY_Controller
{
	...
	function export()
	{
		...
		$query = $this->pl_model->getBreakdownPLexport($groupDate);
		...
		<?php
			...
			foreach ($query->result() as $data) { // Lakukan looping pada variabel sn
				$maskingname = $this->maskingname($data->Debitur_Name);
				...
				$sheet->setCellValue('K' . $numrow, $data->Area);
			...
			header("Content-Disposition: attachment; filename=Detail Achievement PL.xlsx");
