...
class Cc extends MY_Controller
{
	...
	function index()
	{
		...
		//$data['counter'] = $this->cc_model->getCounter($sales_code, $varPos, $tgl);
		$data['breakdown_cc'] = $this->cc_model->breakdown_cc($sales_code, $tgl);
		//load view
		$this->template->set('title','Data Decision CC');
		$this->template->load('template','decision/cc/index', $data);
	}
