...
class Bcas extends MY_Controller
{
    function __construct()
    {
        ...
        $this->load->model('decision/bcas_model', 'model');
    }
    ...
    function index()
    {
        ...
        $period         = $this->model->get_last_period();
        ...
        if ($position == 'DSR') {
            $views              = 'index_dsr';
            $data['getBcas']   = $this->model->get_bcas_dsr($username, $groupDate);
        } else {
            $views = 'index';
        }

        //load view
        $this->template->set('title', 'Decision BCAS');
        $this->template->load('template', 'decision/bcas/' . $views, $data);
    }
    ...
    function get_data()
    {
        ...
        $query = $this->model->get_dataTables($where, $groups, $groupDate);
        ...
        foreach ($query->result() as $row) {
            ...
            $getBcas = $this->model->get_bcas($var, $sales_code, $groupDate);

            $actualOA       = 0 + $getBcas->oa;
            $actualNE       = 0 + $getBcas->ne;

            $actualTotal    = $actualOA + $actualNE;

            if ($position == 'DSR') {
                $data[] = array(
                    ++$no,
                    $sales_code . ', ' . $sales_name . ' (' . $sales_position . ')',
                    '<span class="badge bg-blue">' . number_format($actualNE) . '</span>',
                    '<span class="badge bg-yellow">' . number_format($actualOA) . '</span>',
                    '<span class="badge bg-grey">' . number_format($actualTotal) . '</span>',
                );
            } else {
                $data[] = array(
                    ++$no,
        ...
        $output = array(
            ...
            "recordsTotal"      => $this->model->count_filtered($where, $groups, $groupDate),
            "recordsFiltered"   => $this->model->count_filtered($where, $groups, $groupDate),
            "data"              => $data,
        );
    ...
    function detail($sales, $pos)
    {
        ...
        //load view
        $this->load->view('decision/bcas/detail');
    }
    ...
    function get_data_spv()
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $getBcas = $this->model->get_bcas_detail($var, $sales_code, $var2, $upliner, $groupDate);

            $actualOA    = 0 + $getBcas->oa;
            $actualNE    = 0 + $getBcas->ne;
            $actualTotal = $actualOA + $actualNE; //

            $buttons = in_array($sales_position, $array_structure)
                ? '<a href="javascript:void(0);" onclick="view_spv(\'' . $sales_code . '\',\'' . $sales_position . '\',\'' . $sales_name . '\')" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>'
                : '';
            ...
            $data[] = array(
                ++$no,
                $sales_code . ', ' . $sales_name . ' (' . $sales_position . ')',
                '<span class="badge bg-black">' . number_format($actualOA) . '</span>',
                '<span class="badge bg-blue">' . number_format($actualNE) . '</span>',
                '<span class="badge bg-red">' . number_format($actualTotal) . '</span>',
                $buttons
            );
        }
    ...
    function export_old()
    {
        ...
        $query = $this->model->get_bcas_export($groupDate);
...
<?php
            ...
            foreach ($query->result() as $data) { // Lakukan looping pada variabel sn
                ...
                $sheet->setCellValue('S' . $numrow, $data->Source);
            ...
            header("Content-Disposition: attachment; filename=Data_Achievement_Bcas.xlsx");
    ...
    function export()
    {
        ...
        // 2. Logika jika data KOSONG
        if ($query->num_rows() == 0) {
            ob_end_clean();

            $this->session->set_flashdata('error', 'Tidak ada data untuk diekspor...!!!');

            redirect('decision/bcas');

            return;
        }


        // HANYA BUAT SATU KALI
        $spreadsheet = new Spreadsheet();
        $sheet = $spreadsheet->getActiveSheet();
        $style_col = $this->style_col();
        $style_row = $this->style_row();
        ...
        // Isi data
        $no = 1;
        $numrow = 2; // Mulai dari baris ke-2
        foreach ($query->result() as $data) {
            ...
            $excelDate = \PhpOffice\PhpSpreadsheet\Shared\Date::PHPToExcel($data->Tgl_Open_Account);
            $sheet->setCellValue('N' . $numrow, $excelDate);
            ...
            $sheet->getStyle('A' . $numrow . ':R' . $numrow)->applyFromArray($style_row);
