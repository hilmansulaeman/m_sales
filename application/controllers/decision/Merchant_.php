...
class Merchant extends MY_Controller
{
		...
		foreach ($query->result() as $row) {
			...
			$data[] = array(
				...
				'<a href="javascript:void(0);" onclick="view_spv(\'' . $sales_code . '\',\'' . $sales_position . '\',\'' . $sales_name . '\')" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>
					<a href="javascript:void(0);" onclick="view_detail(\'' . $sales_code . '\',\'' . $sales_position . '\',\'' . $sales_name . '\')" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>'
			);
		}
	...
	function get_data_spv()
	{
		...
		foreach ($query->result() as $row) {
			...
			if (in_array($sales_position, $array_structure)) {
				$buttons = '
					<a href="javascript:void(0);" onclick="view_spv(\'' . $sales_code . '\',\'' . $sales_position . '\',\'' . $sales_name . '\')" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>
					<a href="' . site_url('decision/merchant/detailActualLink/' . $sales_code . '/' . $sales_position . '/' . $sales_name) . '" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>
					';
			} else {
				$buttons = '<a href="' . site_url('decision/merchant/detailActualLink/' . $sales_code . '/' . $sales_position . '/' . $sales_name) . '" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>';
			}
	...
	function det_breakdown_merchant_leader($sales_code, $sales, $mid, $ft, $product, $poplink)
	{
		...
		// $var_code 	= "";
		$groupDate 	= $this->session->userdata('groupDate');
		$data['query'] 	= $this->merchant_model->detBreakdownMerchantLeader($sales_code, $sales, $mid, urldecode($ft), $product, $groupDate);
