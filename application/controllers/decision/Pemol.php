	...
	function get_data()
	{
		// try {
			...
			foreach ($query->result() as $row) {
				...
				// var_dump($getPemol); die;
				// $actualOA  = 0 + $getPemol->oa;
				$actualMY  = 0 + $getPemol->my_oa;
				$actualMB  = 0 + $getPemol->mb_oa;
				// $actualoa = $actualMY+$actualMB;
				// $actualSN  = 0 + $getPemol->sn;
				$actualNE  = 0 + $getPemol->ne;
				...
				$actualTotal = $actualMY + $actualMB + $actualNE + $actualSK + $actualKTB;
				//$target = 120;
				//$gap = $actual - $target;
				...
				$data[] = array(
					...
					// '<span title="Actual" class="badge bg-green">' . number_format($actualoa) . '</span>',
					'<span title="Actual" class="badge bg-black">' . number_format($actualMY) . '</span>',
					'<span title="Actual" class="badge bg-blue">' . number_format($actualMB) . '</span>',
					// '<span title="Actual" class="badge bg-red">'.number_format($actualSN).'</span>',
					'<span title="Actual" class="badge bg-red">' . number_format($actualNE) . '</span>',
		...
		// $this->unit->run($result, "is_true", "Unit Testing view data");
