...
class Deviasi_form extends MY_Controller
{
    ...
    function add()
    {
        ...
        $queryDeviasi = $this->db->get_where('ref_category_structure', array('Category' => 'deviasi'))->row();

        $data['categoryid'] = $queryDeviasi->Category_ID;
        $data['category']   = $queryDeviasi->Category;
        ...
        if (in_array($position_, $list)) {
            $data['sales'] = $this->deviasi_form_model->getData_api($sales_code, $endDate, $startDate, 'bySalesCode');
        } else {
            $data['sales'] = $this->deviasi_form_model->getData_api('', $endDate, $startDate, 'bySalesCode');
        }
    ...
    function getDate($val) {
        ...
        $this->db->where('a.Efective_Date', $val)->where('a.Category', 'DEVIASI')->where('Is_Send', '0');
        $query = $this->db->get();
        $row = $query->row();
