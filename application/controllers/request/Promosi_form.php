...
class Promosi_form extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('request/promosi_form_model');
    }
    ...
    function add()
    {
        ...
        if (in_array($position_, $list)) {
            $data['sales'] = $this->promosi_form_model->get_all_sales_where();
        } else {
            $data['sales'] = $this->promosi_form_model->get_all_sales();
        }
        //load view
        $this->template->set('title', 'Add Form Promosi');
        $this->template->load('template', 'request/prodemo/promosi_form/add', $data);
    }

    function save_request_promosi($uri3, $uri4)
    { //uri3 = add/detail, uri4 = id_request
        ...
        if ($cek > 0) {
            ...
            redirect('request/promosi_form/add');
        } else {
            ...
            redirect('request/promosi_form/detail/' . $request_id);
        }
    }
    ...
    function detail($request_id)
    {
        ...
        $data['request_user'] = $this->promosi_form_model->get_promosi_detail($request_id, $form_id);
        $data['db'] = $this->promosi_form_model->get_by($request_id)->row();

        //load view
        $this->template->set('title', 'Data Request Detail');
        $this->template->load('template', 'request/prodemo/promosi_form/detail', $data);
    }
    ...
    function send($request_id)
    {
        ...
        // $response = $this->promosi_form_model->insert_data_api($data);
    ...
    {
        if (!empty($id)) {
            $query = $this->promosi_form_model->getDetailSales($id);
            if ($query->num_rows() > 0) {
