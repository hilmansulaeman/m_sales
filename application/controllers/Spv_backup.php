...
class Spv extends MY_Controller
{
	...
	function index()
	{
		...
		$sql_gettarget = $this->spv_model->getTarget($level, $position, $product);
		...
		$sqlUnder = $this->spv_model->getDsrUnderPerforms($sales_code, $tgl_1, $product);
	...
	function jml_rts_aktual($sales_code)
	{
		...
		//load view
		$this->template->set('title','Supervisor');
		$this->load->view('spv/jml_rts_aktual', $data);
	}
