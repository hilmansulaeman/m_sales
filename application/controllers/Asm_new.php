...
* 
*/
class Asm_new extends MY_Controller
{
	...
	function index()
	{
		...
		$sqlTarget = $this->asm_model->getTarget($level, $product, $position);
		...
		$this->template->load('template','asm/index_new', $data);
	}
	...
	function jml_apps_aktual($sales_code)
	{
		...
		//load view
		$this->template->set('title','ASM');
		$this->load->view('asm/jml_apps_aktual_new', $data);
	}
	...
	function jml_rts_aktual($sales_code)
	{
		...
		//load view
		$this->template->set('title','ASM');
		$this->load->view('asm/jml_rts_aktual', $data);
	}
