...
{	
	...
	function index()
    {
		...
		if ($position == 'DSR') {
			...
			$data['query'] = $this->cc_model_new->getDataInput($username, $date_from, $date_to, 'ms'); // Mobile Sales
		}else{
			$views = 'index';
		}
		...
		$this->template->load('template','incoming/cc_ms/'.$views,$data);
    }
	...
	function get_data()
	{
		...
		foreach ($query as $row){
			...
			$getIncoming = $this->cc_model_new->getDataInputLeader($var, $sales, $date_from, $date_to, 'ms'); // Mobile Sales
		...
		if($position == 'ASM'){
			...
			$getIncomingDummy = $this->cc_model_new->getDataInputDummy($nik, $date_from, $date_to, 'spv', 'ms'); // Mobile Sales
	...
	function detailSPV($sales, $pos)
	{
		...
		//load view
		$this->load->view('incoming/cc_ms/detailSPV',$data);
	}
	...
	function get_data_spv()
	{
		...
		foreach ($query as $row){
			...
			$data[] = array(
				...
				'<span title="Total DSR Active" class="badge bg-black">'. $total_dsr .'</span>', // total dsr active
				'<span title="Total Input" class="badge bg-red">'. $input .'</span>', // total input
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/SEND/').'" target="_blank"><span title="Total Send" class="badge bg-green">'. $send .'</span></a>', // submit
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/INPROCESS/').'" target="_blank"><span title="Total Inprocess" class="badge bg-info">'. $inprocess .'</span></a>', // inprocess
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/DUPLICATE/').'" target="_blank"><span title="Total Duplicate" class="badge bg-black">'. $duplicate .'</span></a>', // duplicate
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/RTS/').'" target="_blank"><span title="Total RTS" class="badge bg-red">'.$rts .'</span></a>', // rts
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/CANCEL/').'" target="_blank"><span title="Total Cancel" class="badge bg-yellow">'. $cancel .'</span></a>', // cancel by CH
				'<a href="'.site_url('incoming/cc_ms/detail_leader/'.$var.'/'.$sales.'/REJECT/').'" target="_blank"><span title="Total Reject" class="badge bg-red">'. $reject .'</span></a>', // reject by DIKA
				$buttons
			);
		}
		...
		if($pos == 'ASM'){
			...
			$data[] = array(
				...
				'<span title="Total DSR Active" class="badge bg-black">'. $total_dsr_dummy .'</span>', // total dsr active
				'<span title="Total Input" class="badge bg-red">'. $input_dummy .'</span>', // input
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/SEND/').'" target="_blank"><span title="Total Send" class="badge bg-green">'. $send_dummy .'</span></a>', // submit
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/INPROCESS/').'" target="_blank"><span title="Total Inprocess" class="badge bg-info">'. $inprocess_dummy .'</span></a>', // inprocess
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/DUPLICATE/').'" target="_blank"><span title="Total Duplicate" class="badge bg-black">'. $duplicate_dummy .'</span></a>', // duplicate
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/RTS/').'" target="_blank"><span title="Total RTS" class="badge bg-red">'. $rts_dummy .'</span></a>', // rts
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/CANCEL/').'" target="_blank"><span title="Total Cancel" class="badge bg-yellow">'. $cancel_dummy .'</span></a>', // cancel by CH
				'<a href="'.site_url('incoming/cc_ms/detail_leader/t3.ASM_Code/'.$nik.'/REJECT/').'" target="_blank"><span title="Total Reject" class="badge bg-red">'. $reject_dummy .'</span></a>', // reject by DIKA
				'<a href="javascript:void(0);" onclick="view_spv(\''.$nik.'\', `SPV`, `DUMMY SPV`)" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>'
			);
		}
	...
	function view_leader()
    {
		...
		$data['query'] = $this->cc_model_new->getDataInputLeader($data['var'], $username, $date_from, $date_to, 'ms'); // Mobile Sales

		//load view
		$this->template->set('title','Summary Incoming CC MS');
		$this->template->load('template','incoming/cc_ms/view_leader',$data);
    }
	...
	function detail_leader_popup($sales_code, $sales, $status)
	{
		...
		$data['status'] 	= $status;
		$data['query'] 	= $this->cc_model_new->detBreakdownInputLeader($sales_code, $sales, $status, 'ms', $tgl1, $tgl2);
		//load view
		$this->load->view('incoming/cc_ms/detail_leader_popup', $data);
	}
	...
	function detail_leader($sales_code, $sales, $status)
	{
		...
		$this->template->load('template','incoming/cc_ms/detail_leader', $data);
	}
	...
	function detail_dsr($status)
	{
		...
		$data['status'] = $status;
		$data['query'] 	= $this->cc_model_new->detBreakdownInputDSR($sales, $status, 'ms', $tgl1, $tgl2);
		//load view	
		$this->load->view('incoming/cc_ms/detail_dsr', $data);
	}
	...
	// EXPORT PAGE
		...
		function export()
		{
			...
			$query = $this->cc_model_new->getBreakdownExport($date_from, $date_to, 'ms');
			...
			if (count($query) == 0) { ?>
				...
				<?php
				echo "<meta http-equiv='refresh' content='0; url=" . site_url() . "incoming/cc_ms'>";
				...
				header("Content-Disposition: attachment; filename=Data Incoming CC MS.xlsx");
