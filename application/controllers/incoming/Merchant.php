...
class Merchant extends MY_Controller
{
	...
	// function unit_testing_total_dsr()
	// {
	// 	$query = $this->merchant_model->getDataInputLocal('t2.DSR_Code', 'K1200104', '2024-02-01', '2024-02-06', 'IS', '')->total_dsr;
	// 	if ($query == 0) {
	// 		$resultTest = true;
	// 	} else {
	// 		$resultTest = false;
	...
	// 	$this->unit->run($resultTest, 'is_true', 'Total DSR');
	...
	function get_data_new()
	{
		...
		$dsr_position = "('DSR','SPG','SPB')";
		$array_leader = ['BSH', 'RSM', 'ASM', 'SPV'];
		$array_dsr    = ['DSR', 'SPG', 'SPB'];
		...
		$query = $this->merchant_model->get_datatables($where);
		$sales_list = array_map(function($r) {
				return $r->DSR_Code;
			}, $query);
		$var = 't2.DSR_Code'; // default field
		...
		foreach ($query as $row) {
			...
			if (in_array($row->Position, $array_leader)) {
				$buttons = '<a href="javascript:void(0);" onclick="view_spv(\'' . $sales . '\',\'' . $row->Position . '\',\'' . $row->Name . '\')" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>
				<a href="javascript:void(0);" onclick="view_detail(\'' . $sales . '\',\'' . $row->Position . '\',\'' . $row->Name . '\')" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>';
				$var = 't2.' . $row->Position . '_Code';
			} elseif (in_array($row->Position, $array_dsr)) {
				$buttons = '<a href="javascript:void(0);" onclick="view_detail(\'' . $sales . '\',\'' . $row->Position . '\',\'' . $row->Name . '\')" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>';
				$var = 't2.DSR_Code';
			} else {
				$buttons = '';
				$var = 't1.Sales_Code';
			...
			$bulk_summary_data = $this->merchant_model->fetchBulkMerchantSummary($var, $sales_list, $date_from, $date_to, $source);
			$summary = (array)($bulk_summary_data[$sales] ?? []);

			$total_dsr1    = $summary['total_dsr'] ?? 0;
			$actual_totalIS = $summary['total_input'] ?? 0;
			$actual_totalPR = $summary['total_received'] ?? 0;
			$actual_totalPI = $summary['inprocess'] ?? 0;
			$actual_totalRTS = $summary['rts'] ?? 0;
			$actual_totalPS = $summary['send'] ?? 0;
			$actual_totalPC = $summary['cancel'] ?? 0;
			$actual_totalPPS = $summary['pending'] ?? 0;
			
			$data[] = [
				++$no,
				"{$row->DSR_Code}, {$row->Name} ({$row->Position})",
				$row->Branch,
				"<span class='badge bg-green'>{$total_dsr1}</span>",
				"<span class='badge bg-black'>{$actual_totalIS}</span>",
				"<span class='badge bg-info'>{$actual_totalPR}</span>",
				"<span class='badge bg-yellow'>{$actual_totalPI}</span>",
				"<span class='badge bg-red'>{$actual_totalRTS}</span>",
				"<span class='badge bg-green'>{$actual_totalPS}</span>",
				"<span class='badge bg-yellow'>{$actual_totalPPS}</span>",
				"<span class='badge bg-red'>{$actual_totalPC}</span>",
				$buttons
			];
		}
	...
	function get_data_spv()
	{
		...
		foreach ($query as $row) {
			$sales = $row->DSR_Code;
			$safe_name = str_replace(' ', ' - ', trim($row->Name));
			if (in_array($position, $array_leader)) {
				if (in_array($row->Position, $array_leader)) {
					$buttons = '
						<a href="javascript:void(0);" onclick="view_spv(\'' . $sales . '\',\'' . $row->Position . '\',\'' . $row->Name . '\')" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>
						<a href="' . site_url('incoming/merchant/detailActualLink/' . $sales . '/' . $row->Position . '/' . $safe_name) . '" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>';
					$var = 't2.' . $row->Position . '_Code';
				} else {
					$buttons = '<a href="' . site_url('incoming/merchant/detailActualLink/' . $sales . '/' . $row->Position . '/' . $safe_name) . '" target="_blank" class="btn btn-info btn-xs"><i class="fa fa-cog"></i> Detail</a>';
					$var = 't1.Sales_Code';
				}
			} else {
				$buttons = '';
