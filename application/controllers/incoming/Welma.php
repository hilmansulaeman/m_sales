...
{	
	function __construct()
	{
		...
		$this->load->model('incoming/welma_model');
	}
	...
	function index()
    {
		...
		//load view
		$this->template->set('title','Summary Welma');
		$this->template->load('template','incoming/welma/index',$data);
    }
	...
	function get_data()
    {
        ...
        $query = $this->welma_model->get_datatables($where);
        $data = array();
        ...
        foreach ($query as $row){
			...
			$getWelma = $this->welma_model->get_welma($var,$sales,$date_from,$date_to);
			$actual = $getWelma->total;
			$dsr_active = $getWelma->dsr_active;
			$dsr_input = $getWelma->dsr_input;
			//$target = 120;
			//$gap = $actual - $target;
		...
		if($position == 'ASM'){
			$get_welma_dummy_spv = $this->welma_model->get_welma_dummy($nik,$date_from,$date_to,'spv');
			$actual_dummy_spv = $get_welma_dummy_spv->total;
			$dsr_active_dummy = $get_welma_dummy_spv->dsr_active;
			$dsr_input_dummy = $get_welma_dummy_spv->dsr_input;
			$data[] = array(
				++$no,
				'DUMMY SPV',
				'ALL',
				...
				'<span title="Total Input" class="badge bg-green">'.number_format($actual_dummy_spv).'</span>',
        ...
        $output = array(
            ...
            "recordsTotal" => $this->welma_model->count_filtered($where),
            "recordsFiltered" => $this->welma_model->count_filtered($where),
            "data" => $data,
        );
	...
	function detail($sales, $pos)
    {
		...
		//load view
		$this->load->view('incoming/welma/detail');
    }
