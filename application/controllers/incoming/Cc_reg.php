	...
	function index()
	{
		...
		if (isset($date_filter)) {
			...
			if (strtotime($from) > strtotime($to)) {
				$this->session->set_flashdata('error', 'Tanggal From tidak boleh lebih besar dari Tanggal To');
				redirect('incoming/cc_reg');
				return;
			}
		...
		if ($position == 'DSR') {
			...
			$data['query'] = $this->cc_model_new->getDataInput($username, $date_from, $date_to, 'reg');
		} else {
			$views = 'index';
		}
