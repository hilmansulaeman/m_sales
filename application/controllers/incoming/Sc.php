...
{	
	...
	function index()
    {
		...
		if ($position == 'DSR') {
			...
			$data['query'] = $this->sc_model->getDataInput($username, $date_from, $date_to, 'all');
		}else{
			$views = 'index';
		}

		//load view
		$this->template->set('title','Summary Incoming SC');
		$this->template->load('template','incoming/sc/'.$views,$data);
    }
	...
	function get_data()
	{
		...
		$query = $this->sc_model->get_datatables($where);
		$data = array();
		...
		foreach ($query as $row){
			...
			$getIncoming = $this->sc_model->getDataInputLeader($var, $sales, $date_from, $date_to, 'all');
		...
		if($position == 'ASM'){
			...
			$getIncomingDummy = $this->sc_model->getDataInputDummy($nik, $date_from, $date_to, 'spv', 'all');
		...
		$output = array(
			...
			"recordsTotal" => $this->sc_model->count_filtered($where),
			"recordsFiltered" => $this->sc_model->count_filtered($where),
			"data" => $data,
		);
	...
	function detailSPV($sales, $pos)
	{
		...
		//load view
		$this->load->view('incoming/sc/detailSPV',$data);
	}
	...
	function view_leader()
    {
		...
		$data['query'] = $this->sc_model->getDataInputLeader($data['var'], $username, $date_from, $date_to, 'all');

		//load view
		$this->template->set('title','Summary Incoming SC');
		$this->template->load('template','incoming/sc/view_leader',$data);
    }
	...
	function detail_leader_popup($sales_code, $sales, $status)
	{
		...
		$data['status'] 	= $status;
		$data['query'] 	= $this->sc_model->detBreakdownInputLeader($sales_code, $sales, $status, 'all', $tgl1, $tgl2);
		//load view
		$this->load->view('incoming/sc/detail_leader_popup', $data);
	}
	...
	function detail_leader($sales_code, $sales, $status)
	{
		...
		$this->template->load('template','incoming/sc/detail_leader', $data);
	}
	...
	function detail_dsr($status)
	{
		...
		$data['status'] = $status;
		$data['query'] 	= $this->sc_model->detBreakdownInputDSR($sales, $status, 'all', $tgl1, $tgl2);
		//load view	
		$this->load->view('incoming/sc/detail_dsr', $data);
	}
	// END DSR PAGE
	...
	// EXPORT PAGE
		...
		function export()
		{
			...
			$query = $this->sc_model->getBreakdownSCexport($date_from, $date_to);
			...
			if (count($query) == 0) { ?>
				...
				<?php
				echo "<meta http-equiv='refresh' content='0; url=" . site_url() . "incoming/sc'>";
				...
				header("Content-Disposition: attachment; filename=Data Incoming SC.xlsx");
