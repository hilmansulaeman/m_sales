...
{	
	...
	// LEADER PAGE
		...
		function get_data()
		{
			...
			foreach ($query as $row){
				...
				$data[] = array(
					...
					$row->Branch,
					'<span title="Total DSR Active" class="badge bg-green">'.$actual_dsr = ($total_dsr1->total_dsr == null) ? 0 : $total_dsr1->total_dsr.'</span>', // total dsr active
		...
		function detailActual($sales, $varr)
		{
			...
			$data['dataGetDataIS'] = $this->merchant_model->getDataInputLocal($var, $sales, $date_from, $date_to, 'IS'); // IS = Input => input System (total, edc, qris, edc_qris)
			$data['dataGetDataBS'] = $this->merchant_model->getDataInputLocal($var, $sales, $date_from, $date_to, 'BS'); // BS = Input => bukan System (total, edc, qris, edc_qris)

			$data['dataGetDataPR'] 	 = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PR'); // PR = Processing => Process Received
			$data['dataGetDataPI'] 	 = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PI'); // PI = Processing => Process Inprocess
			$data['dataGetDataPRTS'] = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PRTS'); // PRTS = Processing => Processs RTS
			$data['dataGetDataPS'] 	 = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PS'); // PS = Processing => Process Send
			$data['dataGetDataPC'] 	 = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PC'); // PRTS = Processing => Processs Cancel
			$data['dataGetDataPPS']  = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PPS'); // PS = Processing => Process Pending
		...
		function get_data_spv()
		{
			...
			foreach ($query as $row){
				...
				$dataGetDataIS = $this->merchant_model->getDataInputLocal($var, $sales, $date_from, $date_to, 'IS'); // IS = Input => input System
				$dataGetDataBS = $this->merchant_model->getDataInputLocal($var, $sales, $date_from, $date_to, 'BS'); // BS = Input => bukan System
				
				//Total DSR
				$total_dsr1  = $this->merchant_model->getDataInputLocal($var, $sales, $date_from, $date_to, 'IS');

				$dataGetDataPR   = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PR'); // PR = Processing => Process Received
				$dataGetDataPI   = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PI'); // PI = Processing => Process Inprocess
				$dataGetDataPRTS = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PRTS'); // PRTS = Processing => Processs RTS
				$dataGetDataPS   = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PS'); // PS = Processing => Process Send
				$dataGetDataPC 	 = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PC'); // PS = Processing => Process Cancel
				$dataGetDataPPS  = $this->merchant_model->getAppProcessingLocal($var, $sales, $date_from, $date_to, 'PPS'); // PS = Processing => Process Pending
