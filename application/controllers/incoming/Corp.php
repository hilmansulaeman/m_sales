...
class Corp extends MY_Controller
{
	...
	function __construct()
	{
        ...
        $this->load->model('incoming/corp_model');
	}
	...
	function index()
	{
		...
		$data['sql_total'] = $this->corp_model->total($Sales_Code, $var_code, $tgl1, $tgl2);
		$data['breakdown_corp'] = $this->corp_model->m_breakdown_corp($Sales_Code, $var_code, $tgl1, $tgl2);
		
		//load view
		$this->template->set('title','Data Incoming');
		$this->template->load('template','incoming/corp/index', $data);
	}
	...
	function det_breakdown_corp($status)
	{
		...
		$data['query'] = $this->corp_model->m_det_breakdown_corp($Sales_Code, $var_code, $tgl1, $tgl2, $status);
		
		//load view
        $this->load->view('incoming/corp/'.$view, $data);
	}
