...
{	
	...
	function get_data_spv()
	{
		...
		foreach ($query as $row){
			...
			$data[] = array(
				...
				'<a href="'.site_url('incoming/cc_reg/detail_leader/'.$var.'/'.$sales.'/SEND_ACCO/').'" target="_blank"><span title="Total Send Acco" class="badge bg-blue">'. $send_acco .'</span></a>', // send
				...
				'<a href="'.site_url('incoming/cc_reg/detail_leader_ms/'.$var2.'/'.$sales.'/SEND_MS/').'" target="_blank"><span title="Total Send MS" class="badge bg-purple">'. $send_ms .'</span></a>', // send by DIKA
				$buttons
			);
		}
		...
		if($pos == 'ASM'){
			...
			$data[] = array(
				...
				'<a href="'.site_url('incoming/cc_reg/detail_leader/t3.ASM_Code/'.$nik.'/SEND_ACCO/').'" target="_blank"><span title="Total Send" class="badge bg-blue">'. $send_acco_dummy .'</span></a>', // send
				...
				'<a href="'.site_url('incoming/cc_reg/detail_leader_ms/ASM_Code/'.$nik.'/SEND_MS/').'" target="_blank"><span title="Total SEND MS" class="badge bg-red">'. $send_ms_dummy .'</span></a>', // reject by DIKA
				'<a href="javascript:void(0);" onclick="view_spv(\''.$nik.'\', `SPV`, `DUMMY SPV`)" class="btn btn-primary btn-xs"><i class="fa fa-cog"></i> View</a>'
			);
		}
	...
	function detail_leaderMS_popup($sales_code, $sales)
	{
		...
		//load view
		$this->load->view('incoming/cc_reg/detail_leader_ms_popup', $data);
	}
	...
	function detail_leader_ms($sales_code, $sales, $status)
	{
		...
		//load view
		$this->template->set('title','Detail Actual Leader');
		$this->template->load('template','incoming/cc_reg/detail_leader_ms', $data);
	}
