...
class Cc extends MY_Controller
{
	...
	function __construct()
	{
        ...
        $this->load->model('incoming/cc_model');
	}
	...
	function index()
	{
		...
		$data['sql_total'] = $this->cc_model->total($Sales_Code, $var_code, $tgl1, $tgl2);
		$data['breakdown_cc'] = $this->cc_model->m_breakdown_cc($Sales_Code, $var_code, $tgl1, $tgl2);
		
		//load view
		$this->template->set('title','Data Incoming');
		$this->template->load('template','incoming/cc/index', $data);
	}
	...
	function det_breakdown_cc($status)
	{
		...
		$data['query'] = $this->cc_model->m_det_breakdown_cc($Sales_Code, $var_code, $tgl1, $tgl2, $status);
