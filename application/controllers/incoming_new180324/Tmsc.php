...
class Tmsc extends MY_Controller
{
	...
	function get_data()
	{
		...
		if ($position == 'ASM') {
			...
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmsc($nik, $date_from, 'asm', 'ms')->row(); // Mobile Sales
	...
	function get_data_spv()
	{
		...
		if ($pos == 'ASM') {
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmsc($nikSes, $date_from, 'asm', 'ms')->row(); // Mobile Sales
	...
	function view_leader()
	{
		...
		$data['query'] = $this->cc_model_new->get_dataInputLeaderTmsc($data['var2'], $username, $date_from, 'ms')->row(); // Reguler
