...
class Tmcc extends MY_Controller
{
	...
	function index()
	{
		...
		//load view
		$this->template->set('title', 'Summary Incoming TM CC');
		$this->template->load('template', 'incoming_new/tmcc/' . $views, $data);
	...
	function get_data()
	{
		...
		// $this->unit->run($query, "is_array", "Unit Testing Show Data TMCC Filter New");
		// echo $this->unit->report();
		// die;
		...
		foreach ($query->result() as $row) {
			...
			$getIncoming = $this->cc_model_new->get_dataInputLeaderTmcc($var, $sales, $date_from, 'ms')->row();
		...
		if ($position == 'ASM') {
			...
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmcc($nik, $date_from, 'asm', 'ms')->row(); // Mobile Sales
	...
	function detailSPV($sales, $pos)
	{
		...
		//load view
		$this->load->view('incoming_new/tmcc/detailSPV', $data);
	}
	...
	function get_data_spv()
	{
		...
		foreach ($query->result() as $row) {
			...
			$getIncoming = $this->cc_model_new->get_dataInputLeaderTmcc($var, $sales, $date_from, 'ms')->row(); // Mobile Sales
			if (in_array($row->Position, $array_dsr)) {
				$total_dsr = 0;
			} else {
				$total_dsr 	 = $getIncoming->total_ms == NULL ? 0 : $getIncoming->total_ms;
		...
		if ($pos == 'ASM') {
			$getIncomingDummy = $this->cc_model_new->get_dataInputDummyTmcc($nikSes, $date_from, 'asm', 'ms')->row(); // Mobile Sales
	...
	function view_leader()
	{
		...
		$data['query'] = $this->cc_model_new->get_dataInputLeaderTmcc($data['var2'], $username, $date_from, 'ms')->row(); // Reguler

		//load view
		$this->template->set('title', 'Summary Incoming TM CC');
		$this->template->load('template', 'incoming_new/tmcc/view_leader', $data);
	}
	...
	function detail_leader_popup($sales_code, $sales)
	{
		...
		$data['query'] 	= $this->cc_model_new->get_dataDetailTmcc($sales_code, $sales, $tgl1)->result();

		//load view
		$this->load->view('incoming_new/tmcc/detail_leader_popup', $data);
	}
	...
	function detail_leaderMS_popup($sales_code, $sales)
	{
		...
		//load view
		$this->load->view('incoming_new/tmcc/detail_leader_ms_popup', $data);
	}
	...
	function detail_leader($sales_code, $sales, $status)
	{
		...
		$this->template->load('template', 'incoming_new/tmcc/detail_leader', $data);
	}
	...
	function detail_leader_ms($sales_code, $sales, $status)
	{
		...
		//load view
		$this->template->set('title', 'Detail Actual Leader');
		$this->template->load('template', 'incoming_new/tmcc/detail_leader_ms', $data);
	}
	...
	function detail_dsr($status)
	{
		...
		//load view	
		$this->load->view('incoming_new/tmcc/detail_dsr', $data);
		// $result = true;
		// } catch (Exception $e) {
		// 	$result = false;
		// }
	...
	function export()
	{
		...
		$query = $this->cc_model_new->get_dataExportTmcc($position, $nik, $from)->result();
...
<?php
			echo "<meta http-equiv='refresh' content='0; url=" . site_url() . "incoming_new/tmcc'>";
			...
			header("Content-Disposition: attachment; filename=Detail Incoming TM CC.xlsx");
