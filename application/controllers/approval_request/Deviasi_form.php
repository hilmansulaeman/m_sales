...
class Deviasi_form extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('approval_request/deviasi_form_model');
    }
    ...
    function index()
    {
        ...
        $this->template->load('template', 'approval_request/deviasi_form/index');
    }
    ...
    function get_data_deviasi()
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $action = '<a href="' . site_url() . "approval_request/deviasi_form/detail/" . $request_id . '" ><span class="btn btn-xs btn-info"><i class="fa fa-md fa-list" title="Detail Data"></i></span></a>';
    ...
    function detail($request_id)
    {
        ...
        $data['request_user'] = $this->deviasi_form_model->get_deviasi_detail($request_id);
        //$data['db'] = $this->restruct_form_model->get_by($request_id)->row();

        //load view
        $this->template->set('title', 'Data Request Detail');
        $this->template->load('template', 'approval_request/deviasi_form/detail', $data);
    }
    ...
    function approve($request_id)
    {
        ...
        foreach ($forms as $form => $val) {
            ...
            $needapprovalgm = $get_data->Need_Approval_GM;

            $getCategory = $this->db->query("SELECT * FROM data_request WHERE Request_ID = '$request_id'")->row();
            $category = $getCategory->Category;
            $dibuatoleh = $getCategory->Created_By;
            $efective_date = $getCategory->Efective_Date;

            $getnamadibuatoleh = $this->db->query("SELECT Name FROM db_hrd.data_employee WHERE DSR_Code = '$dibuatoleh'")->row();
            $dibuatolehnama = $getnamadibuatoleh->Name;
            ...
            $joindate = $get_data_employee->Join_Date;
            ...
            if (($position == 'BSH' && $needapprovalgm == '2') or ($position == 'GM' && $needapprovalgm == '1')) {
                $data_updates[] = array(
                    'Request_User_ID'   => $request_user_id,
                    'Hit_Code'  => '1009'
                );
                ...
                $data_process_log = array(
                    ...
                    'Request_User_ID'   => $request_user_id,
                    'Hit_Code'          => '1009',
                    'Description'       => 'New Request From ' . $position,
                ...
                $data_logs_hrd[] = array(
                    ...
                    'name'              => $sales_name,
                    'reason'            => $reason,
                    'created_by'        => $dibuatolehnama,
                    'category'          => $category,
                    'join_date'         => $joindate,
                    'efective_date'     => $efective_date
                );

                $this->deviasi_form_model->insert_data_api($data_logs_hrd);
            } else {
        ...
        if ($cek > 0) {
            redirect('approval_request/deviasi_form/detail/' . $request_id);
        } else {
            redirect('approval_request/deviasi_form');
        }
    }
    ...
    function reject($request_id)
    {
        ...
        foreach ($forms as $form => $val) {
            ...
            $this->deviasi_form_model->insert_data_api_update($data_updaterestruct_hrd);
        }
        $this->db->update_batch('internal_sms.data_request_user', $data_update, 'Request_User_ID');
    ...
    {
        if (!empty($id)) {
            $query = $this->deviasi_form_model->getDetailSales($id);
            if ($query->num_rows() > 0) {
