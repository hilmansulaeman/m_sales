...
class Crosselling_form extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('approval_request/crosselling_form_model');
    }
    ...
    function index()
    {
        ...
        $this->template->load('template', 'approval_request/crosselling_form/index');
    }

    function get_data_crosselling()
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $action = '<a href="' . site_url() . "approval_request/crosselling_form/detail/" . $request_id . '" ><span class="btn btn-xs btn-info"><i class="fa fa-md fa-list" title="Detail Data"></i></span></a>';
    ...
    function detail($request_id)
    {
        ...
        $data['request_user'] = $this->crosselling_form_model->get_crosselling_detail($request_id);

        //load view
        $this->template->set('title', 'Data Request Detail');
        $this->template->load('template', 'approval_request/crosselling_form/detail', $data);
    }
    ...
    function approve($request_id)
    {
        ...
        foreach ($forms as $form => $val) {
            ...
            //GET DETAIL SALES
            $getDataEmployee = $this->crosselling_form_model->getData_api($sales_code, 'bySalesCode');
            $employee_id     = $getDataEmployee->Employee_ID;

            // echo '<pre>';
            // print_r($getDataEmployee);
            // echo '</pre>';
            // die();
            ...
            $q_sm = $this->crosselling_form_model->getData_api($username, 'sm_position');
        ...
        if($position == $getApproval->Approval_Max) {
            ...
            // echo '</pre>';
            // die();
            $response = $this->crosselling_form_model->insert_data_api($data_logs_hrd);
        }
        ...
        if ($cek > 0) {
            redirect('approval_request/crosselling_form/detail/' . $request_id);
        } else {
            redirect('approval_request/crosselling_form');
        }
    }
