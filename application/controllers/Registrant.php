...
{	
	...
	function insert_data_reg()
	{
		...
		$data_ = $this->db->query("select id as id from schedule_training where area='".$this->input->post('area')."' and location='".$this->input->post('location')."' and available_date='".$this->input->post('tanggal')."' and time='".$this->input->post('waktu')."'");
		
		$status_schedule = "Register";
		foreach($data_->result() as $items)
		{
			$updateSeatNew = $this->db_model->update_seat_new($items->id);
			$data_request = array(
