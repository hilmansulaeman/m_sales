...
class Restruct_form_model extends CI_Model
{
	...
	private function _get_datatables_query()
    {
        ...
        $where = "a.Form_ID = '2' AND ($checker OR $approval)";
    ...
    function get_restruct_detail($request_id) {
        $username = $this->session->userdata('username');
        return $getData = $this->db->query("SELECT a.*, b.*, c.Employee_ID, e.*
            FROM internal_sms.data_request_user a 
            INNER JOIN internal_sms.set_hit_code b ON b.Hit_Code = a.Hit_Code 
            INNER JOIN internal_sms.data_sales_logs c ON c.Request_User_ID = a.Request_User_ID 
            ...
            WHERE a.Request_ID = '$request_id' AND e.Form_ID = '2' AND d.Sales_Code = '$username' AND d.Status = '0'
