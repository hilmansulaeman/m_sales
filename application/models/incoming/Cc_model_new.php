...
class Cc_model_new extends CI_Model
{
	...
	function getDataProject()
	{
		...
		// if (!$query->status) {
		if (!is_object($query) || !isset($query->status) || $query->status == FALSE) {
			$this->db->distinct();
	...
	function getDataInputLeader($var, $sales, $from, $to, $type, $project = '')
	{
		...
		$encodeProject = urlencode($project);


		// API URL
		$url = $rowAPI->url . 'api/incoming_cc/getDataInputLeader?lead_code=' . $var . '&sales=' . $sales . '&from=' . $from . '&to=' . $to . '&type=' . $type . '&project=' . $encodeProject;
