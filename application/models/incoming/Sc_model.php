...
class Sc_model extends CI_Model
{
	...
	function getBreakdownSCexport($date_from, $date_to)
	{
		$nik 		  = $this->session->userdata('sl_code');
		$position = $this->session->userdata('position');

		// $getAPI = $this->db->query("SELECT * FROM `key_api_dev` WHERE `Description` = 'SC'");
		// $rowAPI = $getAPI->row();
		$rowAPI = $this->get_api();
		...
		$apiPass = $rowAPI->Password;
		// API URL
		$url = $rowAPI->url.'api/incoming_sc/getSCExport?nik='.$nik.'&position='.$position.'&date_from='.$date_from.'&date_to='.$date_to;
