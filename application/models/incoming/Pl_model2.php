...
class Pl_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT COUNT(1) AS pl FROM `internal`.`apps_pl` a INNER JOIN `internal`.`data_sales_copy` b ON a.sales_code=b.DSR_Code WHERE b.$var_code='$Sales_Code' AND a.Created_Date BETWEEN '$tgl1 00:00:00' AND '$tgl2 23:59:59'");
		return $query;
		$query->free_result();
	}
