...
class Tele_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT COUNT(1) AS tele FROM `internal`.`applications` a INNER JOIN `internal`.`data_sales_copy` b ON a.sales_code=b.DSR_Code WHERE b.$var_code='$Sales_Code' AND a.Apply_Card_Code in('7') AND a.Created_Date2 BETWEEN '$tgl1 00:00:00' AND '$tgl2 23:59:59'");
		return $query;
		$query->free_result();
	}
