...
class Pemol_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT COUNT(1) as total, COUNT(DISTINCT(Sales_Code)) as dsr_input,SUM(IF(t1.Source = 'BCA Mobile',1,0)) as bcaM,SUM(IF(t1.Source = 'MyBCA',1,0)) as mBca,
		    (SELECT COUNT(1) FROM internal.data_sales_copy t3 WHERE Position IN ('DSR','SPG','SPB') AND Product = 'PEMOL' AND Status = 'ACTIVE' AND $var = '$sales') as dsr_active
			FROM internal.data_summary_incoming_pemol t1
			LEFT JOIN internal.data_sales_copy t3
			ON t1.Sales_Code = t3.DSR_Code
			WHERE $var = '$sales'
			AND t1.Group_Date = '$from'
		");
		return $query;
		$query->free_result();
	}
	...
	function get_pemol_dummy($sales, $from, $part)
	{
		if ($part == 'rsm') {
			$query = $this->db->query("SELECT count(1) as total, COUNT(DISTINCT(Sales_Code)) as dsr_input,SUM(IF(t1.Source = 'BCA Mobile',1,0)) as bcaM,SUM(IF(t1.Source = 'MyBCA',1,0)) as mBca,
			            (SELECT COUNT(1) FROM internal.data_sales_copy t3 WHERE Position IN ('DSR','SPG','SPB') AND Product = 'PEMOL' AND Status = 'ACTIVE' AND t3.BSH_Code = '$sales' AND t3.RSM_Code = '0') as dsr_active
						FROM internal.data_summary_incoming_pemol t1
			LEFT JOIN internal.data_sales_copy t3
			ON t1.Sales_Code = t3.DSR_Code
						WHERE t3.RSM_Code = '0'
						AND t3.BSH_Code = '$sales'
						AND t1.Group_Date = '$from'
					");
		} else if ($part == 'asm') {
			$query = $this->db->query("SELECT count(1) as total, 
			COUNT(DISTINCT(Sales_Code)) as dsr_input,SUM(IF(t1.Source = 'BCA Mobile',1,0)) as bcaM,SUM(IF(t1.Source = 'MyBCA',1,0)) as mBca,
			(SELECT COUNT(1) FROM internal.data_sales_copy t3 WHERE Position IN ('DSR','SPG','SPB') AND Product = 'PEMOL' AND Status = 'ACTIVE' AND t3.RSM_Code = '$sales' AND t3.ASM_Code = '0') as dsr_active 
			FROM internal.data_summary_incoming_pemol t1
			LEFT JOIN internal.data_sales_copy t3
			ON t1.Sales_Code = t3.DSR_Code
						WHERE t3.ASM_Code = '0'
						AND t3.RSM_Code = '$sales'
						AND t1.Group_Date = '$from'
					");
		} else if ($part == 'spv') {
			$query = $this->db->query(
				"SELECT count(1) as total, 
						COUNT(DISTINCT(Sales_Code)) as dsr_input,SUM(IF(t1.Source = 'BCA Mobile',1,0)) as bcaM,SUM(IF(t1.Source = 'MyBCA',1,0)) as mBca,
			            (SELECT COUNT(1) FROM internal.data_sales_copy t3 WHERE Position IN ('DSR','SPG','SPB') AND Product = 'PEMOL' AND Status = 'ACTIVE' AND t3.ASM_Code = '$sales' AND t3.SPV_Code = '0') as dsr_active 
						FROM internal.data_summary_incoming_pemol t1
						LEFT JOIN internal.data_sales_copy t3
						ON t1.Sales_Code = t3.DSR_Code
						WHERE t3.SPV_Code = '0'
						AND t3.ASM_Code = '$sales'
						AND t1.Group_Date = '$from'"
			);
		}
		return $query;
	...
	// function get_pemol($var, $sales)
	{
		$query = $this->db->query("SELECT * FROM internal.data_summary_incoming_pemol t1
		LEFT JOIN internal.data_sales_copy t3
		ON t1.Sales_Code = t3.DSR_Code
		WHERE $var = '$sales'
		AND t1.Group_Date = '$from'
