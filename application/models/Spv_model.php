...
class Spv_model extends CI_Model
{
	...
	{
		$query = $this->db->query("SELECT COUNT(1) AS total_dsr 
		                           FROM `internal`.`data_sales_structure` 
		                           WHERE SPV_Code='$sales_code' 
								   AND Status='ACTIVE'
								  ");
		return $query;
		$query->free_result();
	...
	function getAproval($sales_code, $product, $tanggal)
	{
		if($product == "CC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_rates FROM `internal`.`application_process` 
							WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1='APPROVED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "EDC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_rates FROM `internal`.`edc_result` 
							WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1 = 'ACCEPTED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "SC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_rates FROM `internal`.`sc_result` 
							WHERE spv_code='$sales_code' AND group_date LIKE '%$tanggal%' AND status_1='APPROVED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "PL")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_rates FROM `internal`.`apps_pl_result` 
							WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1 IN('Approve', 'Approved')");
			return $query;
			$query->free_result();
		}
		elseif($product == "CORP")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_rates FROM `internal`.`corporate_ro_result` 
							WHERE SPV_Code='$sales_code' AND group_date LIKE '%$tanggal%' AND status='APPROVED'");
			return $query;
			$query->free_result();
		}
	}
	
	/*function getRejSpv2($sales_code, $product, $tanggal)
	{
		if($product == "CC")
		{
	...
	function getRejSpv($sales_code, $product, $tanggal)
	{
		if($product == "CC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijeks FROM `internal`.`application_process`
								WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1='DECLINED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "EDC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijeks FROM `internal`.`edc_result`
							WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1 IN('REJECT', 'REJECTED')");
			return $query;
			$query->free_result();
		}
		elseif($product == "SC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijeks FROM `internal`.`sc_result` 
							WHERE spv_code='$sales_code' AND group_date LIKE '%$tanggal%' AND status_1='DECLINED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "PL")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijeks FROM `internal`.`apps_pl_result`
							WHERE SPV_Code='$sales_code' AND Group_Date LIKE '%$tanggal%' AND Status_1='Rejected'");
			return $query;
			$query->free_result();
		}
		elseif($product == "CORP")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijeks FROM `internal`.`corporate_ro_result`
							WHERE SPV_Code='$sales_code' AND group_date LIKE '%$tanggal%' AND status='DECLINED'");
			return $query;
			$query->free_result();
		}
	}
	...
	function getApprovalNas($product, $tanggal)
	{
		if($product == "CC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_nas FROM `internal`.`application_process` 
								WHERE Group_Date LIKE '%$tanggal%' AND Status_1='APPROVED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "EDC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_nas FROM `internal`.`edc_result` 
							WHERE Group_Date LIKE '%$tanggal%' AND Status_1 = 'ACCEPTED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "SC")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_nas FROM `internal`.`sc_result` 
							WHERE group_date LIKE '%$tanggal%' AND status_1='APPROVED'");
			return $query;
			$query->free_result();
		}
		elseif($product == "PL")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_nas FROM `internal`.`apps_pl_result` 
							WHERE Group_Date LIKE '%$tanggal%' AND Status_1 IN('Approve', 'Approved')");
			return $query;
			$query->free_result();
		}
		elseif($product == "CORP")
		{
			$query = $this->db->query("SELECT COUNT(1) as aps_nas FROM `internal`.`corporate_ro_result` 
							WHERE group_date LIKE '%$tanggal%' AND status='APPROVED'");
			return $query;
			$query->free_result();
		}
	}
	...
	function getRejlNas($product, $tanggal)
	{
		if($product == "CC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijek FROM `internal`.`application_process` 
								WHERE Group_Date LIKE '%$tanggal%' AND Status_1='DECLINED'");
			return $query;
			$query->result();
		}
		elseif($product == "EDC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijek FROM `internal`.`edc_result` 
							WHERE Group_Date LIKE '%$tanggal%' AND Status_1 IN('REJECT', 'REJECTED')");
			return $query;
			$query->result();
		}
		elseif($product == "SC")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijek FROM `internal`.`sc_result` 
							WHERE group_date LIKE '%$tanggal%' AND status_1='DECLINED'");
			return $query;
			$query->result();
		}
		elseif($product == "PL")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijek FROM `internal`.`apps_pl_result` 
							WHERE Group_Date LIKE '%$tanggal%' AND Status_1='Rejected'");
			return $query;
			$query->result();
		}
		elseif($product == "CORP")
		{
			$query = $this->db->query("SELECT COUNT(1) as rijek FROM `internal`.`corporate_ro_result` 
							WHERE group_date LIKE '%$tanggal%' AND status='DECLINED'");
			return $query;
			$query->result();
		}
	}
	...
	function cc_inc($sales_code, $tanggal)
	{
		$query = $this->db->query("SELECT 
			...
			AND Apply_Card_Code IN('2','4','7') 
			AND Created_Date LIKE '%$tanggal%'");
		return $query;
		$query->free_result();
	}
	...
	function getDataAppsRtsAkt($sales_code, $product, $tanggal)
	{
		$sql = $this->db->query("SELECT
				...
				FROM `internal`.`data_sales_structure` a WHERE a.SPV_Code='$sales_code' AND a.Status='ACTIVE'");
		return $sql;
		$sql->free_result();
	}

	function getDataAppsRtsAktNew($sales_code, $product, $tanggal)
	{
		$sql = $this->db->query("SELECT
				a.DSR_Code, a.Name, a.Join_Date, a.Efektif_date, a.Product
				FROM `internal`.`data_sales_structure` a WHERE a.SPV_Code='$sales_code' AND a.Status='ACTIVE'");
	...
	function getDsrUnderPerforms($sales_code, $tanggal, $product)
	{
		...
		$query = $this->db->query("SELECT a.DSR_Code, a.Name, b.Product, b.Join_Date 
								 ...
								 AND ratings IN('Under Perform','Under Perform 1', 'Under Perform 2') 
								 AND a.Periode LIKE '%$tanggal%'");
		return $query;
		$query->free_result();
	}
	...
	function getDataRtsDsr($dsr_code, $tgl)
	{
		$sql = $this->db->query("SELECT * FROM `internal_sms`.`tbl_summary_apps_dsr` WHERE DSR_Code='$dsr_code' AND Periode='$tgl'");
		return $sql;
		$sql->free_result();
	}

	function getNamaApplicantCc($dsr_code, $tgl)
	{
		$sql = $this->db->query("SELECT Customer_Name as cs_name, Created_Date2 as tgl_input, (SELECT Name FROM `internal`.`data_sales_copy` WHERE DSR_Code='$dsr_code') as ds_name FROM `internal`.`applications` WHERE Sales_Code='$dsr_code' AND Created_Date2 LIKE '%$tgl%' AND Status='RETURN_TO_SALES'");
		return $sql;
		$sql->free_result();
	}

	function getNamaApplicantEdc($dsr_code, $tgl)
	{
		$sql = $this->db->query("SELECT Merchant_Name as cs_name, Created_Date as tgl_input, (SELECT Name FROM `internal`.`data_sales_copy` WHERE DSR_Code='$dsr_code') as ds_name FROM `internal`.`edc_merchant` WHERE Sales_Code='$dsr_code' AND Created_Date LIKE '%$tgl%' AND Status='RETURN_TO_SALES'");
		return $sql;
		$sql->free_result();
	}

	function getNamaApplicantSc($dsr_code, $tgl)
	{
		$sql = $this->db->query("SELECT CUstomer_Name as cs_name, Created_Date as tgl_input, (SELECT Name FROM `internal`.`data_sales_copy` WHERE DSR_Code='$dsr_code') as ds_name FROM `internal`.`apps_sc` WHERE Sales_Code='$dsr_code' AND Created_Date2 LIKE '%$tgl%' AND Status='RETURN_TO_SALES'");
		return $sql;
		$sql->free_result();
	}
	
	function getKomitmenById($id)
	{
		$sql = $this->db->query("SELECT * FROM `internal_sms`.`tbl_keterangan_sales` WHERE id='$id'");
		return $sql;
		$sql->free_result();
	}
