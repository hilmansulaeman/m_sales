...
class Unblock_form_model extends CI_Model
{
	...
	private function _get_datatables_query()
    {
        ...
        $this->db->join('internal_sms.data_request_user b','b.Request_ID = a.Request_ID', 'LEFT');

        $where = "a.Created_By = '$created_by' AND a.Form_ID = '4' AND a.Created_Date BETWEEN '$tanggal1' AND '$tanggal2'";
	...
	function insert_data_api($data)
	{
		...
		// API URL
		$url = $url_api.'/api/request/add_status';
        // var_dump($data);
    ...
    function get_all_sales() {
        return $getData = $this->db->query("SELECT b.* FROM db_hrd.data_sales_structure a INNER JOIN db_hrd.data_employee b ON b.DSR_Code = a.DSR_Code WHERE a.Status = 'block' AND Position != 'SPV' ORDER BY b.Name ASC");
    }

    function get_all_sales_where() {
        $sales_code = $this->session->userdata('sl_code');
        return $getData = $this->db->query("SELECT b.* FROM db_hrd.data_sales_structure a INNER JOIN db_hrd.data_employee b ON b.DSR_Code = a.DSR_Code WHERE a.ASM_Code = '$sales_code' AND a.Status = 'block' AND a.Position != 'SPV' ORDER BY b.Name ASC");
    }
