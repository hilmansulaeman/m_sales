...
class Promosi_form_model extends CI_Model
{
    ...
    function get_promosi_detail($request_id, $form_id)
    {
    ...
    {
        return $getData = $this->db->query("SELECT t1.*, t2.Employee_ID, t2.DSR_Code FROM db_hrd.data_sales_structure t1 INNER JOIN db_hrd.data_employee t2 ON t2.DSR_Code = t1.DSR_Code WHERE t1.Status = 'ACTIVE' AND t1.Position = 'DSR' ORDER BY t1.Name ASC");
    }

    function get_all_sales_where()
    {
        $sales_code = $this->session->userdata('sl_code');
        return $getData = $this->db->query("SELECT t1.*, t2.Employee_ID, t2.DSR_Code FROM db_hrd.data_sales_structure t1 INNER JOIN db_hrd.data_employee t2 ON t2.DSR_Code = t1.DSR_Code WHERE ASM_Code = '$sales_code' AND t1.Status = 'ACTIVE' AND t1.Position = 'DSR' ORDER BY t1.Name ASC");
    }
    ...
    function insert_data_api($data)
    {
        ...
        $url = $url_api . '/api/request/add_promosi';
