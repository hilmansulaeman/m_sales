...
class exit_meeting_model extends CI_Model
{
    ...
    private function _get_datatables_query()
    {
        ...
        $where = "a.Created_By = '$created_by' AND a.Form_ID = '1' AND a.Created_Date BETWEEN '$tanggal1' AND '$tanggal2'";
    ...
    //=============================================== Monitoring ===============================================
    private function _get_datatables_query_monitoring($nik)
    {
        ...
        $column_order = array(null,'a.DSR_Code','a.Name','a.Branch'); //field yang ada di tabel
        $column_search = array('a.DSR_Code','a.Name','a.Branch'); //field yang diizin untuk pencarian 
        $order = array('a.Name' => 'ASC'); // default order
        $this->db->select('a.*');
        $this->db->from('data_sales_structure a');        
        $this->db->where($where,$nik);
        $this->db->where_in('a.Position',array('DSR','SPG','SPB','Mobile Sales'));        
        $this->db->like('a.Status','active');         
        $this->db->group_by('a.DSR_Code');                
    ...
    function get_datatables_monitoring($nik)
    {
        $this->_get_datatables_query_monitoring($nik);
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query;
    ...
    function count_filtered_monitoring($nik)
    {
        $this->_get_datatables_query_monitoring($nik);
        $query = $this->db->get();
        return $query->num_rows();
    ...
    function get_absent($sales,$from,$to)
    {
        $this->db->select('count(1) as total');
        $this->db->from('data_participant a');
        $this->db->join('ref_schedule b','a.Schedule_ID = b.Schedule_ID');
    ...
    function get_exit_detail($request_id, $form_id)
    {
    ...
    function insert_data_api($data)
    {
        ...
        // API URL
        $url = $url_api . '/api/request/add_exit';
        // echo $url;die();
        ...
        // var_dump($response);
        // die;
        $data = (!empty($response))?$response->data:'';
    ...
    function getData_api($id, $category)
    {
        ...
        $data = (!empty($dataDecode))?$dataDecode->data:'';

        return $data;
    }

    function get_request_user_by_nik($nik)
    {
        $this->db->select('a.*');
        $this->db->from('data_request_user a');
        $this->db->where('a.Sales_Code', $nik);
        return $getData = $this->db->get();
    }
