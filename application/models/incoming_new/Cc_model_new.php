...
class Cc_model_new extends CI_Model
{
	...
	function get_dataExportTmcc($lead_code, $sales, $from)
	{
		...
		$query = $this->db->query("SELECT t1.*,t2.Name FROM internal.data_summary_incoming_cc t1 LEFT JOIN internal.data_sales_copy t2 on t2.DSR_Code = t1.Sales_Code
		WHERE t1.$lead_code = '$sales' AND t1.Group_Date = '$from'");
...
// WHERE t1.$lead_code = '$sales' AND t1.Group_Date = '$from'");
		...
		$query = $this->db->query("SELECT t1.*,t2.Name FROM internal.data_summary_incoming_tmsc t1 LEFT JOIN internal.data_sales_copy t2 on t2.DSR_Code = t1.Sales_Code
WHERE t1.$lead_code = '$sales' AND t1.Group_Date = '$from'");
