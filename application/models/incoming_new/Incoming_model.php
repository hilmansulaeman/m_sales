...
class Incoming_model extends CI_Model
{
	...
	function m_breakdown_qris($Sales_Code, $var_code, $tgl1, $tgl2)
	{
		$query = $this->db->query("SELECT 
				SUM(IF(a.Status='SUBMIT_TO_BCA',1,0)) AS submit_to_bca, 
				SUM(IF(a.Status='SUBMIT_TO_DIKA',1,0)) AS submit_to_dika, 
				...
				FROM `internal`.`edc_merchant` a 
				INNER JOIN `internal`.`data_sales_copy` b ON a.sales_code=b.DSR_Code 
				WHERE b.$var_code='$Sales_Code' AND a.Product_Type='QRIS' AND a.MID_Type = 'NEW MERCHANT' AND a.Created_Date BETWEEN '$tgl1 00:00:00' AND '$tgl2 23:59:59' ");
		return $query;
		$query->free_result();
	}
	
	function m_breakdown_merchant($sales_code, $var_code, $tgl1, $tgl2)
	{
		$query = $this->db->query("SELECT 
					SUM(IF(a.Hit_Code IN ('101','102') AND a.Product_Type = 'QRIS',1,0)) AS input_qris, 
					SUM(IF(a.Hit_Code IN ('101','102') AND a.Product_Type = 'EDC',1,0)) AS input_edc, 
					SUM(IF(a.Hit_Code IN ('101','102') AND a.Product_Type = 'EDC_QRIS',1,0)) AS input_edcqris,
					SUM(IF(a.Hit_Code IN ('101','102'),1,0)) AS total_input,
					SUM(IF(a.Hit_Code IN ('103','104','107') AND a.Product_Type = 'QRIS',1,0)) AS rts_qris,
					SUM(IF(a.Hit_Code IN ('103','104') AND a.Product_Type = 'EDC',1,0)) AS rts_edc,
					SUM(IF(a.Hit_Code IN ('103','104') AND a.Product_Type = 'EDC_QRIS',1,0)) AS rts_edcqris,
					SUM(IF(a.Hit_Code IN ('103','104','107'),1,0)) AS total_rts,
					SUM(IF(a.Hit_Code = '105' AND a.Product_Type = 'QRIS',1,0)) AS received_qris,
					SUM(IF(a.Hit_Code = '105' AND a.Product_Type = 'EDC',1,0)) AS received_edc,
					SUM(IF(a.Hit_Code = '105' AND a.Product_Type = 'EDC_QRIS',1,0)) AS received_edcqris,
					SUM(IF(a.Hit_Code = '105',1,0)) AS total_received,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'QRIS' AND a.Status IN('SUBMIT_TO_DIKA','PENDING_FU'),1,0)) AS inprocess_qris,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'EDC' AND a.Status IN('SUBMIT_TO_DIKA','PENDING_FU'),1,0)) AS inprocess_edc,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'EDC_QRIS' AND a.Status IN('SUBMIT_TO_DIKA','PENDING_FU'),1,0)) AS inprocess_edcqris,
					SUM(IF(a.Hit_Code = '106' AND a.Status IN('SUBMIT_TO_DIKA','PENDING_FU'),1,0)) AS total_inprocess,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'QRIS' AND a.Status IN('SUBMIT_TO_BCA','RESUBMIT_TO_BCA'),1,0)) AS send_qris,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'EDC' AND a.Status IN('SUBMIT_TO_BCA','RESUBMIT_TO_BCA'),1,0)) AS send_edc,
					SUM(IF(a.Hit_Code = '106' AND a.Product_Type = 'EDC_QRIS' AND a.Status IN('SUBMIT_TO_BCA','RESUBMIT_TO_BCA'),1,0)) AS send_edcqris,
					SUM(IF(a.Hit_Code = '106' AND a.Status IN('SUBMIT_TO_BCA','RESUBMIT_TO_BCA'),1,0)) AS total_send,
					COUNT(1) AS total
					FROM `internal`.`edc_merchant` a 
					INNER JOIN `internal`.`data_sales_copy` b ON a.Sales_Code=b.DSR_Code 
					WHERE b.$var_code='$sales_code' 
					AND(
					CASE WHEN a.Hit_Code IN('101','102') THEN a.Created_Date BETWEEN '$tgl1 00:00:01' AND '$tgl2 23:59:59'
						WHEN a.Hit_Code = '105' THEN a.Received_Date BETWEEN '$tgl1 00:00:01' AND '$tgl2 23:59:59'
						ELSE a.Decision_Date BETWEEN '$tgl1 00:00:01' AND '$tgl2 23:59:59'
					END)
				");
		return $query;
		$query->free_result();
	...
	function m_det_breakdown_qris($Sales_Code, $var_code, $tgl1, $tgl2, $status)
	{
		if($status == "others")
		{
			$filStatus = " AND a.Status Not in('SUBMIT_TO_BCA', 'SUBMIT_TO_DIKA', 'RETURN_TO_SALES', 'RETURN_TO_DIKA', 'RETURN_FROM_BCA', 'RESUBMIT_TO_BCA', 'REJECT', 'PROJECT', 'CANCEL')";
		...
		$query = $this->db->query("SELECT a.Merchant_Name, a.Created_Date, b.Name, b.SPV_Name, b.ASM_Name, b.RSM_Name
				FROM `internal`.`edc_merchant` a 
				INNER JOIN `internal`.`data_sales_copy` b ON a.sales_code=b.DSR_Code 
				WHERE b.$var_code='$Sales_Code' AND a.Product_Type='QRIS' AND a.MID_Type = 'NEW MERCHANT' 
				AND a.Created_Date BETWEEN '$tgl1 00:00:00' AND '$tgl2 23:59:59' $filStatus 
				ORDER BY b.SPV_Name ASC
				");
