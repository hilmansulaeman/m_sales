...
class Cc_model extends CI_Model
{
	...
	private function _get_dataTable_query($where, $groups, $groupDate)
	{
		...
		// $this->db->order_by("
		// 'CASE WHEN $group_code = 0 THEN $group_name END' 'DESC', 
		// 'CASE WHEN $group_code != 0 THEN $group_name END' 'ASC'
		// ");
	...
	function get_datatablesexport($where, $groups, $groupDate)
	{
		$this->_get_dataTable_query($where, $groups, $groupDate);
	...
	function breakdown_cc_dsr($sales_code, $group_date)
	{
		$query = $this->db->query("SELECT 
			...
			COALESCE(SUM(IF(Status_1 = 'ADD ON',1,0)),0) as add_on
	...
	function breakdown_cc($sales_code, $group_date)
	{
	...
	function breakdown_cc_detail($sales_code, $var, $upliner, $group_date)
	{
		$query = $this->db->query("SELECT 
			COUNT(1) as count_app_cc,
			COALESCE(SUM(IF(Status_1 = 'APPROVED',1,0)),0) as approve,
	...
	function getBreakdownCc($sales_code, $status, $tgl)
	{
		if ($status == "APPROVE") {
		...
		} elseif ($status == "ADD_ON") {
			$filter = " AND Status_1 = 'ADD ON'";
		} else {
	...
	function getBreakdownCcexport($tgl)
	{
	...
	{
		$query = $this->db->query("SELECT Group_Date FROM internal.application_process ORDER BY Group_Date DESC LIMIT 1");
		return $query;
		$query->free_result();
	}
}
