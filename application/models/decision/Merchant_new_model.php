...
class Merchant_new_model extends CI_Model
{
	...
	private function _get_dataTable_query($where, $groups, $groupDate)
	{
		...
		if($position == 'BSH'){
			$co = array(
				null,'rsm_code','rsm_name','asm_code','asm_name','spv_code','spv_name','sales_code','sales_name','area','position'
			);

			$cs = array(
				'rsm_code','rsm_name','asm_code','asm_name','spv_code','spv_name','sales_code','sales_name','area','position'
			);
		} else if($position == 'RSM'){
			$co = array(
				null,'asm_code','asm_name','spv_code','spv_name','sales_code','sales_name','area','position'
			);

			$cs = array(
				'asm_code','asm_name','spv_code','spv_name','sales_code','sales_name','area','position'
			);
		} else if($position == 'ASM'){
			$co = array(
				null,'spv_code','spv_name','sales_code','sales_name','area','position'
			);

			$cs = array(
				'spv_code','spv_name','sales_code','sales_name','area','position'
			);
		} else {
			$co = array(
				null,'sales_code','sales_name','area','position'
			);

			$cs = array(
				'sales_code','sales_name','area','position'
			);
		}
		...
		$column_search = $cs; 
		$this->db->select('*');
		$this->db->from('`internal`.`summary_merchant`');
		if ($where) {
			$this->db->where($where);
		}
		$this->db->where("periode", $groupDate);
	...
	function get_summary($var, $sales, $group_date, $product,$st)
	{
		$nik 		= $this->session->userdata('sl_code');
		$position 	= $this->session->userdata('position');
		$pos_renew = strtolower($position);
		$upPosition = $pos_renew . '_code';
		if($st == 'index'){
			$where = $this->db->where($upPosition, $nik);
		} else {
			if($position == 'DSR'){
				$where = $this->db->where('Sales_Code', $nik);;
			} else {
				$where = $this->db->where($upPosition, $nik);
			}
		}
		...
		if($product == 'EDC'){
			$select = 'SUM(new_kredit) as new_kredit,
					   SUM(new_nonkredit) as new_nonkredit,
					   SUM(existing_kredit) as existing_kredit,
					   SUM(existing_nonkredit) as existing_nonkredit,
					   SUM(reject) as reject';
		} else {
			$select = 'SUM(new) as new,
					   SUM(existing) as existing,
					   SUM(reject) as reject';
		}
		...
		$this->db->like('category', $product);
		$this->db->where("periode", $group_date);
		$where;
		$query = $this->db->get();
		return $query->row();
	...
	function detBreakdownMerchantLeader($sales_code, $sales, $mid, $ft, $product, $groupDate)
	{
		...
		if($product == 'EDC'){	
			if ($mid == 'reject') {
				...
				$where = $this->db->where("Facilities_Type2", $ft);
			}
		} else {
			$where = "";
		}
		...
		$this->db->where("MID_Type", $mid_);
		$this->db->where("Group_Date", $groupDate);
		$where;
		$this->db->order_by('Sales_Name','ASC');
		$query = $this->db->get();
	...
	function calculate_point($product, $mid, $total)
	{
		// $query = $this->db->query("SELECT * FROM `set_point` WHERE Product = '$product' AND MID_Type = '$mid'");
		$this->db->select('*');
		$this->db->from('set_point');
		$this->db->where('Product', $product);
		$this->db->where('MID_Type', $mid);
		$query = $this->db->get();
